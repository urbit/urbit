...
Regular symbols follow this pattern:
    ...
    prefix    purpose                      .h            .c
    -------------------------------------------------------------
    u3a_      allocation                   i/noun/a.h    noun/a.c
    u3e_      persistence                  i/noun/e.h    noun/e.c
    u3h_      hashtables                   i/noun/h.h    noun/h.c
    u3i_      noun construction            i/noun/i.h    noun/i.c
    u3j_      jet control                  i/noun/j.h    noun/j.c
    u3m_      system management            i/noun/m.h    noun/m.c
    u3n_      nock computation             i/noun/n.h    noun/n.c
    u3r_      noun access (error returns)  i/noun/r.h    noun/r.c
    u3t_      profiling                    i/noun/t.h    noun/t.c
    u3v_      arvo                         i/noun/v.h    noun/v.c
    u3x_      noun access (error crashes)  i/noun/x.h    noun/x.c
    u3z_      memoization                  i/noun/z.h    noun/z.c
    u3k[a-g]  jets (transfer, C args)      i/jets/k.h    jets/[a-g]/*.c
    u3q[a-g]  jets (retain, C args)        i/jets/q.h    jets/[a-g]/*.c
    u3w[a-g]  jets (retain, nock core)     i/jets/w.h    jets/[a-g]/*.c

Irregular symbols always start with `u3` and obey no other rules.
They're defined in `i/noun/aliases.h`.  Finally, `i/all.h` includes
...
in `jets/tree.c`.  The data structures:
...
> 1. All arms of a jetting core must be implemented in C.
> 2. The parent of a jetted core must also be jetted.

> This would force extremely conservative use of just, which seems
> like a good constraint, given that jet-proliferation compromises
> our other design goals (reliability, upgradability, simplicity).
...
precisely, in `jets/dash.c`.  We declare the hierarchy as a tree
...
`jets/`, follows the tier and the function name.

For instance, `++add` is `u3wa_add(cor)`, `u3qa_add(a, b)`, or
`u3ka_add(a, b)`, in `jets/a/add.c`.  `++get` in `++by` is
`u3wdb_get(cor)`, `u3kdb_get(a, b)`, etc, in `jets/d/by_get.c`.

For historical reasons, all internal jet code in `jets/[a-f]`
...
compiler jets (`jets/f/*.c`).  Unfortunately, it's probably not
