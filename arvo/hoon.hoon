...
::    ?:(=(~ r.gen) ~ (hail(gen [%all r.gen]) 6))
    ...
    ::
        ...
        {?($sggl $thin) *}  [%tsgl [%hint p.gen [%$ 1]] q.gen]
        {?($sgbc $live) *}  [%hint [%live [%rock %$ p.gen]] q.gen]
    ...
    ::
        {?($tsfs $ram) *}  [%var q.gen p.gen r.gen]
        {?($tsdt $set) *}
