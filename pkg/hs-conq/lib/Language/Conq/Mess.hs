...
module Language.Conq.Mess where
...
infer = \case
  ...
  ELazy   -> throwE "infer: EPush" -- TODO
  EMark   -> throwE "infer: EHash" -- TODO

-- An Alternate Encoding for Conq ----------------------------------------------

data Flag = T | F
  deriving (Generic, Flat)

data Hint = HLazy | HMark
  deriving (Generic, Flat)
...
expAxe = \case
  ...
  ECase x y -> ACase (expAxe x) (expAxe y)
  EType ty  -> undefined
  EMark     -> AHint HMark
  ELazy     -> AHint HMark
  ETent ref -> expAxe (valFor (getGrain ref))
  EQuot e   -> APure (axeVal (expAxe e))
--EUnquot e -> undefined
...
step s = \case
  ...
  ELazy             -> case s of
                         ...
                         _        -> (VV, ESubj)
  EMark             -> (putGrain s, ESubj)
  EQuot e           -> (forVal e, ESubj)
...
spin :: Exp
spin = call (valExp spinFn) (EPure VN)
