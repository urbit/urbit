...
=>  |%  ++  dispatch-thread
        ...
        ++  migrate
          ...
          =/  dry=?
            ...
            ?>  =(%migr term)
            test
          [%arvo %syscall %a %mate `ship dry]
        ::
        ++  send-ahoy
          |=  [=ship test=?]
          ^-  card
          =/  =path  ?:(test /test/mesa-2 /mesa-2)
          [%pass (dispatch-flow %send ship test) %arvo %syscall %a %plea ship %$ path %ahoy ~]
    ::
    --
::
|=  [=bowl:gall sat=state]
...
++  on-load
  ...
  ?:  ?=(%0 -.old)
    ...
    ::
    $(old old(- %1), this (emit %pass /ames %arvo %syscall %a %load %ames))
  ?>  ?=(%1 -.old)
  this(sat old)
::  handle ahoy actions:
::
