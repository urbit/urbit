...
/+    hall-lib=hall, hall-legacy, default-agent         ::  libraries
/=    seed  /~  !>(.)
/=    filter-gram
      /^  $-({telegram:hall-sur bowl:mall} telegram:hall-sur)
      /|  /:  /%/filter  /!noun/
          /~  |=({t/telegram:hall-sur bowl:mall} t)
      ==
/=    custom-rules
      /^  (map knot $-({ship ?($r %w) bowl:mall} ?))
      /:  /%/rules  /_  /!noun/
::
::::
=,  hall-sur
...
=;  hall-core
  ...
  ++  handle-init
    ^-  step:agent:mall
    =^  cards  hall-core  (~(prep hall-core bowl stat) ~)
    [cards this(stat +<+.hall-core)]
  ::
  ...
  ++  handle-upgrade-state
    ...
    =^  cards  hall-core  (~(prep hall-core bowl stat) `states)
    [cards this(stat +<+.hall-core)]
  ::
  ++  handle-poke
    |=  [=mark =vase]
    ^-  step:agent:mall
    =^  cards  hall-core
      =/  t  ~(. hall-core bowl stat)
      ?+  mark  ~|([%hall-bad-mark mark] !!)
          %hall-command  (poke-hall-command:t !<(command vase))
          %hall-action   (poke-hall-action:t !<(action vase))
          %hall-save     (poke-hall-save:t !<(name vase))
          %load-legacy   (poke-load-legacy:t !<(name vase))
          %hall-load     (poke-hall-load:t !<(name vase))
          %hall-log      (poke-hall-log:t !<(name vase))
          %hall-unlog    (poke-hall-unlog:t !<(name vase))
          %import        (poke-import:t !<(* vase))
  ...
  ++  handle-subscribe
    |=  =path
    ^-  step:agent:mall
    =^  cards=(list card:agent:mall)  hall-core
      (~(peer hall-core bowl stat) path)
    [cards this(stat +<+.hall-core)]
  ::
  ++  handle-unsubscribe
    |=  =path
    ...
    =^  cards=(list card:agent:mall)  hall-core
      ?+  path  `hall-core
        [%circle *]  (~(pull-circle hall-core bowl stat) +.path)
      ==
    [cards this(stat +<+.hall-core)]
  ::
  ++  handle-peek
  ...
  ++  handle-agent-response
    ...
    =^  cards  hall-core
      ...
      ?-    -.gift
        ...
        ?.  ?=([%repeat *] wire)
          `hall-core
        (coup-repeat:t +.wire p.gift)
      ::
          %subscription-ack  (reap:t wire p.gift)
          %subscription-close
        ?-  wire
          [%circle *]  (quit-circle:t +.wire)
          *            (quit:t wire)
        ==
      ::
          %subscription-update
        ?+  p.cage.gift  ~|([%hall-bad-sub-up-mark wire p.cage.gift] !!)
...
++  da
  ...
  ::  that aren't pokes.
  ::  where appropriate, creates cards. those get
  ...
  ++  da-done
    ::    resolve core
    ::
    ::  produces the cards stored in ++da's cards.
    ::  they are produced in reverse order because
  ...
  ++  da-emil
    ::    emit card list
    ::
    ::  adds multiple cards to the head of {cards}.
  ...
  ++  da-emit
    ::    emit a card
    ::
    ::  adds a card to the head of {cards}.
    ::
    ~/  %hall-da-emit
    |=  mov/card
    %_(+> cards [mov cards])
  ...
  ++  sa
    ...
    ++  sa-abjure
      ...
      =/  wir
        ...
        [cir ran]
      [%pass wir %agent [hos.cir dap.bol] %unsubscribe ~]
    ::
    ++  sa-eject
...
++  wire-to-peer
  ::    peer card from wire
  ::
  ::  builds the peer card associated with the wire.
  ::
  ~/  %hall-wire-to-peer
  |=  wir/wire
  ^-  card
  =+  tar=(wire-to-target wir)
  [%pass wir %agent [p.tar dap.bol] %subscribe q.tar]
::
++  wire-to-target
  ::    ship+path from wire
  ::
...
++  gentle-quit
  ...
  |=  {who/ship pat/path}
  ^-  (list card)
  =/  qer=query  (path-to-query pat)
  ?.  ?=($circle -.qer)  ~
  ?.  =(who our.bol)  [%give %subscription-close `pat `who]~
  %-  zing
  ...
  :~  :*  %pass  /
          ...
          %poke  %hall-action
          !>([%source n | [[[our.bol nom.qer] ran.qer] ~ ~]])
      ==
    ::
      :*  %pass
...
++  affection
  ...
  =<  cards
  %+  roll  ~(tap by sup.bol)
  |=  [[d=duct s=ship p=path] cards=(list (list card)) already=(set path)]
  ^-  [(list (list card)) (set path)]
  ?:  =([%export ~] p)
    [~ already]
  =+  qer=(path-to-query p)
  =/  first=(list card)
    ...
    ==
      ...
      ::
      ::  XX we might be sending them anyway because the next line may
      ::  still fire on another subscription
      ~
    [%give %subscription-update `p %hall-rumor !>(u.rum)]~
  =?  already  ?=(^ first)  (~(put in already) p)
  :_  already
  :_  cards
  %+  welp  first
  ?.  ?=($circle -.qer)  ~
  ::  kill the subscription if we forgot the story.
  ?.  (~(has by stories) nom.qer)  ~&  >  %gentle-1  (gentle-quit s p)
  ::  kill the subscription if it's past its range.
  =-  ?:(done:- ~&(> %gentle-2 (gentle-quit s p)) ~)
  %.  ran.qer
...
++  peer
  ...
  ?:  ?=([%export *] pax)
    ...
    :*  %give
        ...
        ~
        %export
        !>([%hall-v0 grams:(~(got by stories) ~.inbox)])
    ==
  ::
  ...
  :_  mos
  [%give %subscription-update ~ %hall-prize !>(u.u.piz)]
::
++  pull-circle
...
++  poke-noun
  ...
  ?:  =(a 'refederate')
    ...
    ?:  =(bov our.bol)  ~
    :~  [%pass /burden %agent [bov dap.bol] %unsubscribe ~]
        (wire-to-peer /burden)
    ==
  ?:  =(a 'incoming')
  ...
  ?:  =(a 'kill-all-subs')
    :_  +>
    %-  ~(rep by sup.bol)
    |=  [[d=duct =ship =path] out=(list card)]
    [[%give %subscription-close `path `ship] out]
  ::
  ?:  =(a 'kill-our-subs')
    :_  +>
    [%give %subscription-close ~ `our.bol]~
  ::
  ?:  =(a 'print')
