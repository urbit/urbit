...
/+    *hall, sole-lib=sole, default-agent               ::  libraries
/=    seed  /~  !>(.)
::
::::
  ::
...
=;  talk-core
  ...
  ++  handle-init
    ^-  step:agent:mall
    =^  cards  talk-core  (~(prep talk-core bowl stat) ~)
  ...
  ++  handle-upgrade-state
    |=  old-state=vase
    ^-  step:agent:mall
    =/  =states  !<(states old-state)
    =^  cards  talk-core  (~(prep talk-core bowl stat) `states)
    [cards this(stat +<+.talk-core)]
  ::
  ++  handle-poke
    |=  [=mark =vase]
    ^-  step:agent:mall
    =^  cards  talk-core
      ?+  mark  ~|([%talk-bad-mark mark] !!)
          %noun  (~(poke-noun talk-core bowl stat) !<(@t vase))
          %sole-action
        %-  ~(poke-sole-action talk-core bowl stat)
        !<(sole-action:sole-sur vase)
      ==
    [cards this(stat +<+.talk-core)]
  ::
  ++  handle-subscribe
  ...
  ++  handle-agent-response
    ...
    ^-  step:agent:mall
    =^  cards  talk-core
      =/  t  ~(. talk-core bowl stat)
      ?-    -.gift
          %http-response       !!
          %poke-ack            (coup-client-action:t +>.wire +.gift)
          %subscription-ack    `talk-core
          %subscription-close
        ?+  wire  ~|([%talk-bad-sub-close-wire wire] !!)
          [%server %client *]  (quit-server-client:t +>.wire)
          [%server %inbox *]   (quit-server-inbox:t +>.wire)
        ==
      ::
          %subscription-update
        ?+  p.cage.gift  ~|([%talk-bad-sub-up-mark wire p.cage.gift] !!)
          %hall-prize  (diff-hall-prize:t wire !<(prize q.cage.gift))
          %hall-rumor  (diff-hall-rumor:t wire !<(rumor q.cage.gift))
        ==
      ==
    [cards this(stat +<+.talk-core)]
  ::
...
++  prep
  ...
  ::
      $0
    =.  mos  [[%pass /server/inbox %agent server %unsubscribe ~] peer-inbox mos]
...
++  ta
  ...
  ::  state, or create cards. these cards get produced
  ::  upon finalizing the core's with with ++ta-done.
  ::  when making changes to the shell, the ++sh core is
  ::  used.
  ::
  |_  ::  cards:  cards created by core operations.
      ::  sole-effects:  sole effects created by core operations
      ::
      $:  cards/(list card)
          sole-effects/(list sole-effect:sole-sur)
      ==
  ::
  ::  #  %resolve
  +|  %resolve
  ...
  ++  ta-done
    ::    resolve core
    ::
    ::  produces the cards stored in ++ta's cards.
    ::  %sole-effect cards get squashed into a %mor.
    ::
    ^+  [*(list card) +>]
    :_  +>
    =/  foc/(unit sole-effect:sole-sur)
      ?~  sole-effects  ~
      ?~  t.sole-effects  `i.sole-effects               ::  single sole-effect
      `[%mor (flop sole-effects)]                       ::  more sole-effects
    ::  produce cards or sole-effects and cards.
    ?~  foc  (flop cards)
    [[%give %subscription-update `/sole %sole-effect !>(u.foc)] (flop cards)]
  ::
  ::  #
  ::  #  %emitters
  ::  #
  ...
  ++  ta-emil
    ::    emit card list
    ::
    ::  adds multiple cards to the core's list.
    ::  flops to emulate ++ta-emit.
    ::
    |=  mol/(list card)
    %_(+> cards (welp (flop mol) cards))
  ::
  ++  ta-emit
    ::  adds a card to the core's list.
    ::
    |=  mov/card
    %_(+> cards [mov cards])
  ::
  ...
  ++  sh
    ...
    ::
    ++  sh-fact
      ::  adds a console effect to ++ta's cards.
      ::
      |=  fec/sole-effect:sole-sur
      ^+  +>
      +>(sole-effects [fec sole-effects])
    ::
    ++  sh-act
      ::  adds an action to ++ta's cards.
      ::
      |=  act/action
      ^+  +>
      %=  +>
    ...
    ++  sh-prod
      ::    show prompt
      ::
      ::  makes and stores a card to modify the cli
...
++  poke-noun
  ...
  ?:  =(a 'reconnect')
    ...
    :_  +>
    :~  [%pass /server/client %agent server %unsubscribe ~]
        [%pass /server/inbox %agent server %unsubscribe ~]
        peer-client
        peer-inbox
    ==
  ?:  =(a 'reset')
