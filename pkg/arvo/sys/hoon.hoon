...
++  up
  ...
  ++  qor
    ...
    ++  jab                                             ::  update at key k
      ~/  %jab
      |*  [a=pro =k f=$-((unit (pair @ v)) (pair * (unit (pair @ v))))]
      =/  bon  _=>((f ~) -)
      ^-  (pair bon pro)
      =/  ped  (pet a k)
      =?  a  ?=(^ ped)
    ...
    ++  jib                                             ::  update at min-pri
      ~/  %jib
      |*  [a=pro f=$-((unit (trel k @ v)) (pair * (unit (trel k @ v))))]
      =/  bon  _=>((f ~) -)
      ^-  (pair bon pro)
      =/  vue  (bot a)
      ?~  vue
  ...
  ++  jab                                               ::  alter
    ...
    =/  val=(pair bon pro)  (jab:qor t.buc k f)
    :-  p.val
...
++  ut
  ...
  ++  caching-ergo
    ...
    =;  [lef=(pair (list (pair type foot)) (list (pair axis nock))) grun=_grub]
      =^  lit  grub  (caching-fire(grub grun) p.lef)
  ...
  ++  caching-et
    ...
    ++  mint
      ...
      =;  [par=(pair type nock) grun=_grub]
        ?.  vet
          [par grun]
        =^  nests  grub  (caching-nest(sut gol, grub grun) & p.par)
        ?>  nests
        [par grub]
      (caching-ergo p.lug rig)
    ::
    ++  mull
      |=  [gol=type dox=type]
      ^-  [[type type] _grub]
