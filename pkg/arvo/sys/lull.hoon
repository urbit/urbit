...
++  xmas
  ...
  ++  name
    ...
    ++  en
      ...
      =/  typ  (dec (met 3 p.tap))
      =/  loq  ?:(=(13 boq) 0 1)
      =/  fag  ?:(=(num 0) [p=1 q=0] =-([p=(met 3 -) q=-] (end 5 num)))
      :+  bloq=3
        [s+~ 0 [2 ran] [2 ryf] [1 typ] [1 loq] [2 ?:(=(p.fag 0) 0 (dec p.fag))] ~]
  ...
  +$  task
    ...
    $%  $>(%vega vane-task)
        ...
        [%make-poke p=spar:ames q=path]    :: initiate %poke request XX combined with $<(%page mess) ? XX make encrypted request a la %chum / %keen+^
    ==
  +$  gift
    $%  [%sent p=(list lane:pact) q=(pair @ @)]     :: send a request/response packet
