...
::
+$  yoke  $%([[%zuse %416] yoke-416] [[%zuse %417] yoke-417])
::
+$  yoke-416
  $:  control-duct=duct
      run-nonce=@t
...
::
+$  egg  $%([[%zuse %416] egg-416] [[%zuse %417] egg-417])
...
++  mo
  ...
  ++  mo-ap-abed
    |=  [dap=term =routes]
    ::  ^-  $%([[%zuse %418] _ap-418] [[%zuse %417] _ap-417])
    (mo-ap-yoke dap routes (~(got by yokes.state) dap))
  ::
  ++  mo-ap-yoke
    |=  [dap=term =routes =yoke]
    ::  ^-  $%([[%zuse %418] _ap-418] [[%zuse %417] _ap-417])
    ?-  -.yoke
      [%zuse %417]  [[%zuse %417] (ap-yoke:ap-417 dap routes +.yoke)]
      [%zuse %416]  [[%zuse %416] (ap-yoke:ap-416 dap routes +.yoke)]
    ==
  ...
  ++  mo-doff
    ...
    |-  ^+  mo-core
    ?~  apps  mo-core
    =/  ap-core  (mo-ap-yoke p.i.apps [~ our] q.i.apps)
  ...
  ++  mo-receive-core
    ~/  %mo-receive-core
    |=  [dap=term bek=beak agent=agent-any]
    ^+  mo-core
    ::
    ...
    ?^  yak
      ...
      =/  ap-core  (mo-ap-abed dap `our)
      ~&  [%dap dap]
      ~&  [%ap-core -.ap-core]
      ~&  [%agent -.agent]
      =/  ap-core-2
        ?-  -.ap-core
          [%zuse %417]  ?>  ?=([%zuse %417] -.agent)  (ap-reinstall:ap-core +.agent)
          [%zuse %416]  ?>  ?=([%zuse %416] -.agent)  (ap-reinstall:ap-core +.agent)
        ==
      ::  =.  ap-core  (ap-reinstall:ap-core agent)
      =.  mo-core  ap-abet:ap-core-2
      (mo-clear-queue dap)
    ::
    =.  yokes.state
  ...
  ++  mo-breach
    ...
    =.  mo-core
      ...
      =/  app  (mo-ap-abed name.i.agents routes)
  ...
  ++  mo-handle-sys-lag
    ...
    ?>  ?=([%ames %clog *] sign-arvo)
    ::
    =/  agents=(list [term yoke])  ~(tap by yokes.state)
    |-  ^+  mo-core
    ?~  agents  mo-core
    ::
    =.  mo-core
      =/  app  (mo-ap-abed -.i.agents `our)
  ...
  ++  mo-handle-use
    ...
    ?.  ?=([?(%gall %behn) %unto *] sign-arvo)
      ...
      =/  app
        ...
        =/  =routes  [disclosing=~ attributing=ship]
        (mo-ap-abed dap routes)
      ::
      =/  app2  (ap-generic-take:app t.t.t.wire sign-arvo)
      ap-abet:app2
    ?>  ?=([%out @ @ *] t.t.wire)
    ...
    ?:  ?=(%| -.agent.u.yoke)
      ...
      ==
    =/  app  (mo-ap-abed dap routes)
    =/  app2  (ap-specific-take:app t.t.wire unto)
    ap-abet:app2
  ...
  ++  mo-idle
    ...
    ?~  yoke
      ...
      mo-core
    ap-abet:ap-idle:(mo-ap-abed dap `our)
  ...
  ++  mo-nuke
    ...
    =.  mo-core  ap-abet:ap-nuke:(mo-ap-abed dap `our)
    mo-core(yokes.state (~(del by yokes.state) dap))
  ::  +mo-load: install agents
  ::
  ++  mo-load
    |=  agents=(list [=dude =beak agent=agent-any])
    =.  mo-core
      |-  ^+  mo-core
      ?~  agents  mo-core
  ...
  ++  ap-417
    ...
    ++  ap-abet
      ^+  mo-core
      ::
      =/  running  (~(put by yokes.state) agent-name [[%zuse %417] yoke])
      =/  moves
    ...
    ++  ap-nuke
      ...
      =^  maybe-tang  ap-core  (ap-ingest ~ |.([will *agent-417]))
      ap-core
    ...
    ::
    ++  ap-mule
      |=  run=_^?(|.(*step:agent-417))
      ^-  (each step:agent-417 tang)
      =/  res  (mock [run %9 2 %0 1] (look rof ~))
      ?-  -.res
        %0  [%& !<(step:agent-417 [-:!>(*step:agent-417) p.res])]
        %1  [%| (smyt ;;(path p.res)) ~]
        %2  [%| p.res]
      ==
    ...
    ::
    ++  ap-ingest
      |=  [ack=?(%poke-ack %watch-ack ~) run=_^?(|.(*step:agent-417))]
      ^-  [(unit tang) _ap-core]
      =/  result  (ap-mule run)
    ...
    ++  ap-handle-result
      ~/  %ap-handle-result
      |=  result=(each step:agent-417 tang)
      ^-  [(list move) _ap-core]
      ?:  ?=(%| -.result)
        `ap-core
      ::
  ...
  ++  ap-416
    ...
    ++  ap-abet
      ^+  mo-core
      ::
      =/  running  (~(put by yokes.state) agent-name [[%zuse %416] yoke])
      =/  moves
        =/  giver  |=(report=(each suss tang) [hen %give %onto report])
        =/  from-suss  (turn agent-config giver)
        :(weld agent-moves from-suss moves)
    ...
    ++  ap-nuke
      ...
      =/  will=(list card:agent-416)
        ...
        |=  [[=wire =dock] ? =path]
        [%pass wire %agent dock %leave ~]
      =^  maybe-tang  ap-core  (ap-ingest ~ |.([will *agent-416]))
      ap-core
    ::  +ap-from-internal: internal move to move.
    ::
    ::    We convert from cards to duct-indexed moves when resolving
    ...
    ::
    ++  ap-mule
      |=  run=_^?(|.(*step:agent-416))
      ^-  (each step:agent-416 tang)
      =/  res  (mock [run %9 2 %0 1] (look rof ~))
      ?-  -.res
        %0  [%& !<(step:agent-416 [-:!>(*step:agent-416) p.res])]
        %1  [%| (smyt ;;(path p.res)) ~]
        %2  [%| p.res]
      ==
    ::  +ap-mule-peek: same as +ap-mule but for (unit (unit cage))
    ...
    ::
    ++  ap-ingest
      |=  [ack=?(%poke-ack %watch-ack ~) run=_^?(|.(*step:agent-416))]
      ^-  [(unit tang) _ap-core]
      =/  result  (ap-mule run)
      =^  new-moves  ap-core  (ap-handle-result result)
      =/  maybe-tang=(unit tang)
    ...
    ++  ap-handle-result
      ~/  %ap-handle-result
      |=  result=(each step:agent-416 tang)
      ^-  [(list move) _ap-core]
      ?:  ?=(%| -.result)
        `ap-core
      ::
