...
::
+$  yoke  $%([[%zuse %417] yoke-417] [[%zuse %418] yoke-418])
::
+$  yoke-417
  $:  control-duct=duct
      run-nonce=@t
...
::
+$  egg  $%([[%zuse %417] egg-417] [[%zuse %418] egg-418])
...
++  mo
  ...
  ++  mo-ap-yoke
    ...
    ::  ^-  $%([[%zuse %418] _ap-418] [[%zuse %417] _ap-417])
    ?-  -.yoke
      [%zuse %418]  [[%zuse %418] (ap-yoke:ap-418 dap routes +.yoke)]
  ...
  ++  mo-receive-core
    ...
    ?^  yak
      ...
      =/  ap-core-2
        ?-  -.ap-core
          [%zuse %418]  ?>  ?=([%zuse %418] -.agent)  (ap-reinstall:ap-core +.agent)
  ...
  ++  ap-418
    ...
    ++  ap-abet
      ^+  mo-core
      ::
      =/  running  (~(put by yokes.state) agent-name [[%zuse %418] yoke])
      =/  moves
    ...
    ++  ap-nuke
      ...
      =^  maybe-tang  ap-core  (ap-ingest ~ |.([will *agent-418]))
      ap-core
    ...
    ::
    ++  ap-mule
      |=  run=_^?(|.(*step:agent-418))
      ^-  (each step:agent-418 tang)
      =/  res  (mock [run %9 2 %0 1] (look rof ~))
      ?-  -.res
        %0  [%& !<(step:agent-418 [-:!>(*step:agent-418) p.res])]
        %1  [%| (smyt ;;(path p.res)) ~]
        %2  [%| p.res]
      ==
    ...
    ::
    ++  ap-ingest
      |=  [ack=?(%poke-ack %watch-ack ~) run=_^?(|.(*step:agent-418))]
      ^-  [(unit tang) _ap-core]
      =/  result  (ap-mule run)
    ...
    ++  ap-handle-result
      ~/  %ap-handle-result
      |=  result=(each step:agent-418 tang)
      ^-  [(list move) _ap-core]
      ?:  ?=(%| -.result)
        `ap-core
      ::
