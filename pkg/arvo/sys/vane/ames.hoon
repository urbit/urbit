...
=>  |%
    ...
    ++  ames-helper
      ...
      ::
      ++  is-our-bulk
        |=  [our=ship ames-state=axle:mesa =balk]
        ^-  ?
        =-  ~?  =(| -)
      ...
      ::
      ++  check-fine-key
        |=  [ames-state=axle:mesa =balk key-idx=@]
        ^-  ?
...
=>  |%
    ++  ames
      ...
      =>  ~%  %per-event  ..trace  ~
          ...
          ++  ev
            =|  moves=(list move)
            :: ~%  %event-gate  ..ev  ~
            |=  [[now=@da eny=@ rof=roof] =duct ames-state=axle:mesa]
      ...
      ++  load
        =<  |=  $=  old
                $%  [%4 %adult state=ames-state-4]
                    [%5 %adult state=ames-state-5]
                    [%6 %adult state=ames-state-6]
                    [%7 %adult state=ames-state-7]
                    [%8 %adult state=ames-state-8]
                    [%9 %adult state=ames-state-9]
                    [%10 %adult state=ames-state-10]
                    [%11 %adult state=ames-state-11]
                    [%12 %adult state=ames-state-12]
                    [%13 %adult state=ames-state-13]
                    [%14 %adult state=ames-state-14]
                    [%15 %adult state=ames-state-15]
                    [%16 %adult state=ames-state-16]
                    [%17 %adult state=ames-state-17]
                    [%18 %adult state=ames-state-18]
                    [%19 %adult state=ames-state-19]
                    [%20 %adult state=ames-state-20]
                    [%21 %adult state=ames-state-21]
                    axle:mesa
                ==
            =|  moz=(list move)
            |-  ^+  vane-gate
            ...
            ?:  ?=(%19 -.old)
              ...
              =.  moz
                ...
                :_  moz
                [unix-duct.state.old %give %saxo get-sponsors:ev-core]
    ...
    ++  mesa
      ...
      =>  ::  inner event-handling
          ...
          =|  moves=(list move)
          ::
          |_  [[now=@da eny=@uvJ rof=roof] hen=duct per=[=ship sat=ship-state]]
          ::
          +|  %helpers
          ::
          ++  ev-core  .
          ...
          ++  ev-req-plea
            ...
            ^+  ev-core
            =/  ship-state  (~(get by chums.ames-state) ship)
            ::
            ?.  ?=([~ %known *] ship-state)
          ...
          ++  ev-pact-poke
            ...
            ::
            =/  ship-state  (~(get by chums.ames-state) her.poke-name)
            ::
            ?.  ?=([~ %known *] ship-state)
          ...
          ++  ev-make-mess
            ...
            ^+  ev-core
            =/  her  (~(gut by chums.ames-state) ship.p *ship-state)
          ...
          ++  ev-put-per
            |=  =ship
            ^+  ames-state
            ames-state(chums (~(put by chums.ames-state) ship known/*peer-state))
          ::
          ++  ev-got-duct
            |=  =bone
            ^-  duct
          ...
          ++  sy  ::  system/internal: %born, %heed, %kroc, %prod...
            ...
            ++  sy-publ
              ...
              ++  on-publ-breach
                ...
                ::
                =.  chums.ames-state
                  (~(put by chums.ames-state) ship [%known peer-state])
                ::
                :: =.  ev-core
                ::   %-  ev-emit
                ::   :*  unix-duct.ames-state  %give  %nail  ship
                ::       (get-forward-lanes our peer-state chums.ames-state)
                ::   ==
                ::  if one of our sponsors breached, give the updated list to vere
                ::
              ...
              ++  on-publ-sponsor
                ...
                ?~  sponsor
                  ...
                  sy-core
                ::
                =/  state=(unit ship-state)  (~(get by chums.ames-state) ship)
                ?.  ?=([~ %known *] state)
                  %-  (slog leaf+"ames: missing peer-state, ignoring" ~)
                  sy-core
                =.  sponsor.+.u.state   u.sponsor
                ...
                ::   %-  ev-emit
                ::   :*  unix-duct.ames-state  %give  %nail  ship
                ::       (get-forward-lanes our +.u.state chums.ames-state)
                ::   ==
                ::
                sy-core
              ::  +on-publ-full: handle new pki data for peer(s)
              ...
              ++  on-publ-full
                ...
                |^  ^+  sy-core
                    ...
                    ?.  (~(has by keys.point) life.point)
                      $(points t.points)
                    ::
                    =/  old-ship-state  (~(get by chums.ames-state) ship)
                    ::
              ...
              ++  insert-peer-state
                ...
                ::
                =.  chums.ames-state
                  (~(put by chums.ames-state) ship ship-state)
                ::
                :: =?  ev-core  ?=(%czar (clan:title ship))
                ::   %-  ev-emit
                ::   :*  unix-duct.ames-state  %give  %nail  ship
                ::       (get-forward-lanes our +.ship-state chums.ames-state)
                ::   ==
                sy-core
              --
            ::
