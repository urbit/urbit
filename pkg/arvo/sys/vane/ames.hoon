          ...
          ++  ev
            ...
            ++  on-keen
              ...
              ?:  ?=([~ %known *] ship-state)
                ?~  sec
                  =<  abet
                  (on-keen:(abed-peer:pe ship +.u.ship-state) fist path duct)
              ...
              ++  on-publ-full
                ...
                ++  meet-alien-ship
                  ...
                  =^  moves  ames-state
                    ...
                    %-  ~(rep in ducts)
                    |=  [=duct c=_cor]
                    (on-keen:c (~(has in sizes.todos) path) path duct)
                  ::
...
=>  ::  per-peer core-routing migration check
    ...
    ++  pe-cancel
      ...
      ::
      ?:  ?=(%ames -.ship-state)
        (call:ames hen ~ soft+yawn/all^spar)
      =^  moves  ames-state
        =<  ev-abet
        ?.  ?=([~ %known *] +.ship-state)
          ::  XX delete from alien agenda?
...
++  call
  ...
  ?-    -.task
    ...
    ::  common tasks
    ::
      ?(%plea %cork %keen %yawn %wham %load %whit)
...
++  load
  ...
  ++  state-21-to-0
    ...
    %=  old
        chain
      [server-chain=chain.old priv=sec:ex:crypto-core.old chums=~ core=%ames]
    ::
        peers
      %-  ~(run by peers.old)
      |=  s=ship-state-21
      ^-  ship-state
      ?.  ?=(%known -.s)
        s(chums [chums.s sizes=~])
      s(chain [chain.s sizes=~])
    ==
  --
::  +scry: dereference namespace
::
