...
++  per-server-event
  ...
  ++  request
    ...
    ?-    -.action
      ...
      =/  ski       (rof tick ~ /eyre %ca bek sup)
      =/  cag=cage  (need (need ski))
      ?>  =(%vase p.cag)
      =/  gat=vase  !<(vase q.cag)
      =/  res=toon
        %-  mock  :_  (look rof tick ~ /eyre)
        :_  [%9 2 %0 1]  |.
        %+  slam
          %+  slam  gat
  ...
  ++  handle-scry
    ...
    ++  do-scry
      |=  [care=term =desk =path]
      ^-  (unit (unit cage))
      (rof tick ~ /eyre care [our desk da+now] path)
    ::
    ++  error-response
      |=  [status=@ud =tape]
      ^-  (quip move server-state)
  ...
  ++  authentication
    ...
    ++  code
      ^-  @ta
      =/  res=(unit (unit cage))
        (rof tick ~ /eyre %j [our %code da+now] /(scot %p our))
  ...
  ++  by-channel
    ...
    ++  app-to-desk
      ...
      =/  des=(unit (unit cage))
        (rof tick ~ /eyre %gd [our app.u.sub da+now] /$)
      ?.  ?=([~ ~ *] des)
    ...
    ++  channel-event-to-json
      ...
      =/  [from=(unit [=desk =mark]) jsyn=(unit sign:agent:gall)]
        ...
        =/  convert=(unit vase)
          =/  cag=(unit (unit cage))
            (rof tick ~ /eyre %cf [our desk.event da+now] /[have]/json)
          ?.  ?=([~ ~ *] cag)  ~
          `q.u.u.cag
        ?~  convert
