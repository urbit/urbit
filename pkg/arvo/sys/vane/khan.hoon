...
::
++  poke-spider
  |=  [hen=duct =dude:gall =cage]
  ^-  move
  [hen %pass //g %g %deal [our our /khan/[dude]] %spider %poke cage]
::
++  watch-spider
  |=  [hen=duct =dude:gall =path]
  ^-  move
  [hen %pass //g %g %deal [our our /khan/[dude]] %spider %watch path]
--
=|  khan-state
=*  state  -
...
++  call
  ...
  ::
      %fard  (bard hen dude.task 'khan-fyrd--' bear.p.task %| [name args]:p.task)
      %lard  (bard hen dude.task 'khan-lard--' bear.task %& shed.task)
      %fyrd
    =*  fyd         p.task
    ...
    [%k %fard dude.task bear.fyd name.fyd p.q.args.fyd vase]
  ==
::
++  bard
  |=  [hen=duct =dude:gall prefix=@ta =bear payload=(each shed [name=term args=cage])]
  ^-  [(list move) _khan-gate]
  ...
  =.  tic  +(tic)
  :_  khan-gate
  :~  (watch-spider hen dude /thread-result/[tid])
      (poke-spider hen dude cage)
  ==
::
