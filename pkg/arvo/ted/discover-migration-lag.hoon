...
=/  az
  ...
  ==
::
=/  lockup-safes=(map @p [safe=address from=address])
  %-  ~(gas by *(map @p [address address]))
  ^-  (list [@p [@ux @ux]])
  :~
    :+  ~bec
...
=/  m  (strand ,vase)
^-  form:m
~&  %checking-lockup-table
=/  locks=(list [gal=@p safe=@ux from=@ux])  ~(tap by lockup-safes)
|-
=*  loop-loc  $
?^  locks
  =,  i.locks
  ::  check the lockup
  ::
  ;<  =old=batch:linear:az  bind:m
    (batches:linear:az from)
  ;<  =new=batch:linear:az  bind:m
    (batches:linear:az safe)
  ~?  =(amount.old-batch amount.new-batch)
    [%so-strange-lockup-transfer gal old=from new=safe]
  ~?  &(=(0 amount.new-batch) !=(safe approved.old-batch))
    [%awaiting-lock-approval g=gal l=safe now=approved.old-batch]
  ~?  &(=(0 amount.new-batch) =(safe approved.old-batch))
    [%awaiting-lock-transfer g=gal l=safe]
  loop-loc(locks t.locks)
::  checkk galaxies and lockups
::
~&  %galaxies
|-
=*  loop-gax  $
?^  gax
  ...
  =/  spawn-proxy=(unit address)
    ...
    ?:  (lth remaining 50)
      `shallow-safe
    `deep-safe
  ~?  !=(management-proxy.deed shallow-safe)
    [%awaiting-management-proxy gal]
  ~?  !=(voting-proxy.deed proxy-safe)
    [%awaiting-----voting-proxy gal]
  ~?  &(?=(^ spawn-proxy) !=(spawn-proxy.deed u.spawn-proxy))
    [%awaiting------spawn-proxy gal]
  ~?  !=(owner.deed deep-safe)
    [%awaiting---------transfer gal]
  loop-gax(gax t.gax)
::
::  handle stars
::
...
?^  saz
  ...
  ~?  ?&  !=(transfer-proxy.deed linear-star-release:mainnet-contracts)
          !=(management-proxy.deed shallow-safe)
      ==
    [%awaiting-management-proxy star]
  ~?  ?&  !=(transfer-proxy.deed linear-star-release:mainnet-contracts)
          !=(spawn-proxy.deed proxy-safe)
      ==
    [%awaiting------spawn-proxy star]
  ~?  &(!=(owner.deed deep-safe) !=(owner.deed shallow-safe))
    [%awaiting---------transfer star]
  loop-saz(saz t.saz)
::
(pure:m !>(~))
