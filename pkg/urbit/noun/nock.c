...
_n_melt(u3_noun ops,
        ...
        c3_w*   mem_w)
{
  c3_w    len_w = u3qb_lent(ops), i_w = len_w - 1, a_w;
  c3_y    cod_y;
 ...
 */
static u3n_prog*
_n_prog_new(c3_w byc_w, c3_w cal_w, c3_w reg_w, c3_w lit_w, c3_w mem_w)
{
  c3_w cab_w = (sizeof(u3j_site) * cal_w), reb_w = (sizeof(u3j_rite) * reg_w),
       lib_w = (sizeof(u3_noun) * lit_w), meb_w = (sizeof(u3n_memo) * mem_w),
...
_n_prog_asm(u3_noun ops, u3n_prog* pog_u, u3_noun sip)
{
  ...
  c3_s    lit_s = 0, cal_s = 0, mem_s = 0, reg_s = 0;
...
_n_prog_from_ops(u3_noun ops)
{
  ...
  u3n_prog* pog_u;
  c3_w      byc_w = 1, // HALT
    cal_w = 0, reg_w = 0, lit_w = 0, mem_w = 0;
...
static c3_w
_n_comp(u3_noun*, u3_noun, c3_o, c3_o);
  ...
  if ( c3n == u3du(hif) ) {
    ...
    switch ( hif ) {
      ...
      case c3__bout: {
        ...
        ++nef_w;
        _n_emit(&fen, (c3y == los_o) ? HILL : HILK);
        // skip over the cleanup opcode
        ++nef_w;
        _n_emit(&fen, u3nc(SBIP, 1));

        //  call hilt_fore
        //  HILB overflows to HILS
        ++tot_w;
        _n_emit(ops, u3nc(HILB, u3nc(u3k(hif), u3k(nef))));
        // if fore return c3n, skip fen
        ++tot_w;
        _n_emit(ops, u3nc(SBIN, nef_w));
        tot_w += nef_w;
        _n_apen(ops, fen);
        // post-skip cleanup opcode
        ++tot_w;
        _n_emit(ops, (c3y == los_o) ? TOSS : SWAP);
      } break;
    }
  }
  else {
    ...
    switch ( zep ) {
      default: {
        ...
        switch ( zep ) {
          ...
          case c3__bout: {
            ...
            // add appropriate hind opcode
            ++nef_w;
            _n_emit(&fen, (c3y == los_o) ? HINL : HINK);
            // skip over the cleanup opcode
            ++nef_w;
            _n_emit(&fen, u3nc(SBIP, 1));
            ...
            ++tot_w;
            _n_emit(ops, u3nc(HINB, u3nc(u3k(zep), u3k(nef))));
            // if fore return c3n, skip fen
            ++tot_w;
            _n_emit(ops, u3nc(SBIN, nef_w));
            tot_w += nef_w;
      ...
      case c3__spot:
        tot_w += _n_comp(ops, hod, c3n, c3n);
        ++tot_w;
        _n_emit(ops, u3nc(BUSH, zep)); // overflows to SUSH
      ...
      case c3__fast:
        ...
        ++tot_w;
        _n_emit(ops, (c3y == los_o) ? BALT : BAST);
        break;
      ...
      case c3__memo: {
        ...
        ++tot_w;
        _n_emit(ops, u3nt(op_y, mem_w, u3k(nef)));
        tot_w += mem_w;
        _n_apen(ops, mem);
        break;
      }
...
_n_formulaic(u3_noun fol)
{
  ...
  if ( c3y == u3du(op) ) {
    ...
    switch ( op ) {
      ...
      case 8:
      case 12:
        return (c3y == u3r_cell(ar, &a, &b)) && _n_formulaic(a)
               && _n_formulaic(b);
      case 6:
        return (c3y == u3r_trel(ar, &a, &b, &c)) && _n_formulaic(a)
               && (_n_formulaic(b) || _n_formulaic(c));
      case 9:
        return (c3y == u3r_cell(ar, &a, &b)) && (c3y == u3ud(a))
               && _n_formulaic(b);
      case 10:
...
_n_comp(u3_noun* ops, u3_noun fol, c3_o los_o, c3_o tel_o)
{
  ...
  if ( c3y == u3du(cod) ) {
    ...
    ++tot_w;
    _n_emit(ops, (c3y == los_o) ? AULT : AUTO);
  }
  else
    switch ( cod ) {
      case 0:
        ...
        switch ( arg ) {
          ...
          case 2:
            ++tot_w;
            _n_emit(ops, (c3y == los_o) ? HELD : HEAD);
            break;
          case 3:
            ++tot_w;
            _n_emit(ops, (c3y == los_o) ? TALL : TAIL);
            break;
          default:
            op_y = (c3y == los_o)
                     ? (arg <= 0xFF     ? FABL
                        : arg <= 0xFFFF ? FASL
                                        : FIBL) // overflows to FISL
                     : (arg <= 0xFF     ? FABK
                        : arg <= 0xFFFF ? FASK
                                        : FIBK); // overflows to FISK
            ++tot_w;
            _n_emit(ops, u3nc(op_y, u3k(arg)));
            break;
        }
        break;
        ...
        switch ( arg ) {
          ...
          default:
            op_y = (c3y == los_o)
                     ? (arg <= 0xFF     ? LILB
                        : arg <= 0xFFFF ? LILS
                                        : LIBL) // overflows to LISL
                     : (arg <= 0xFF     ? LITB
                        : arg <= 0xFFFF ? LITS
                                        : LIBK); // overflows to LISK
            ++tot_w;
            _n_emit(ops, u3nc(op_y, u3k(arg)));
            break;
        }
      ...
      case 2:
        ...
        op_y = (c3y == tel_o) ? NOCT : ((c3y == los_o) ? NOLK : NOCK);
        ++tot_w;
        _n_emit(ops, op_y);
        break;
      ...
      case 5: {
        ...
        if ( c3n == u3du(hed) ) {
          ...
          if ( hec_t && tec_t ) {
            ...
            tot_w += _n_comp(ops, tel, c3n, c3n);
            ++tot_w;
            _n_emit(ops, (c3y == los_o) ? SALM : SAME);
          }
          else {
      ...
      case 6: {
        u3_noun mid, yep = u3_nul, nop = u3_nul;
        c3_w    yep_w, nop_w;
        c3_t    yep_t, nop_t;
        ...
        ++yep_w;
        _n_emit(&yep, u3nc(SBIP, nop_w));
        ++tot_w;
        _n_emit(ops, u3nc(SBIN, yep_w));
        tot_w += yep_w;
        _n_apen(ops, yep);
        tot_w += nop_w;
        _n_apen(ops, nop);
      ...
      case 8:
        ...
        tot_w += _n_comp(ops, hed, c3n, c3n);
        ++tot_w;
        _n_emit(ops, (c3y == los_o) ? SNOL : SNOC);
        tot_w += _n_comp(ops, tel, c3y, tel_o);
        break;
      ...
      case 10: {
        ...
        switch ( axe ) {
          ...
          default:
            op_y = (c3y == los_o)    ? (axe <= 0xFF) ? MUTB
                                       : (axe <= 0xFFFF)
                                         ? MUTS
                                         : MITB // overflows to MITS
                   : (axe <= 0xFF)   ? KUTB
                   : (axe <= 0xFFFF) ? KUTS
                                     : KITB; // overflows to KITS
      ...
      case 12:
        ...
        tot_w += _n_comp(ops, tel, c3n, c3n);
        ++tot_w;
        _n_emit(ops, (c3y == los_o) ? WILS : WISH);
        break;
...
{
  c3_y one = buf[(*ip_w)++], two = buf[(*ip_w)++], tre = buf[(*ip_w)++],
       qua = buf[(*ip_w)++];
...
_n_burn(u3n_prog* pog_u, u3_noun bus, c3_ys mov, c3_ys off)
{
  /* OPCODE TABLE */
  static void* lab[] = {
    &&do_halt, &&do_bail, &&do_copy, &&do_swap, &&do_toss, &&do_auto, &&do_ault,
    &&do_snoc, &&do_snol, &&do_head, &&do_held, &&do_tail, &&do_tall, &&do_fabk,
    &&do_fask, &&do_fibk, &&do_fisk, &&do_fabl, &&do_fasl, &&do_fibl, &&do_fisl,
    &&do_lit0, &&do_lit1, &&do_litb, &&do_lits, &&do_libk, &&do_lisk, &&do_lil0,
    &&do_lil1, &&do_lilb, &&do_lils, &&do_libl, &&do_lisl, &&do_nolk, &&do_noct,
    &&do_nock, &&do_deep, &&do_bump, &&do_sam0, &&do_sam1, &&do_samb, &&do_sams,
    &&do_sanb, &&do_sans, &&do_same, &&do_salm, &&do_samc, &&do_sbip, &&do_sips,
    &&do_swip, &&do_sbin, &&do_sins, &&do_swin, &&do_kicb, &&do_kics, &&do_ticb,
    &&do_tics, &&do_wils, &&do_wish, &&do_bush, &&do_sush, &&do_drop, &&do_heck,
    &&do_slog, &&do_bast, &&do_sast, &&do_balt, &&do_salt, &&do_skib, &&do_skis,
    &&do_slib, &&do_slis, &&do_save, &&do_hilb, &&do_hils, &&do_hinb, &&do_hins,
    &&do_hilk, &&do_hill, &&do_hink, &&do_hinl, &&do_muth, &&do_kuth, &&do_mutt,
    &&do_kutt, &&do_musm, &&do_kusm, &&do_mutb, &&do_muts, &&do_mitb, &&do_mits,
...
_cn_take_prog_dat(u3n_prog* dst_u, u3n_prog* src_u)
{
  ...
  for ( i_w = 0; i_w < src_u->cal_u.len_w; ++i_w ) {
    u3j_site_take(&(dst_u->cal_u.sit_u[i_w]), &(src_u->cal_u.sit_u[i_w]));
  }

  for ( i_w = 0; i_w < src_u->reg_u.len_w; ++i_w ) {
    u3j_rite_take(&(dst_u->reg_u.rit_u[i_w]), &(src_u->reg_u.rit_u[i_w]));
  }
}
...
_cn_merge_prog_dat(u3n_prog* dst_u, u3n_prog* src_u)
{
  ...
  for ( i_w = 0; i_w < src_u->cal_u.len_w; ++i_w ) {
    u3j_site_merge(&(dst_u->cal_u.sit_u[i_w]), &(src_u->cal_u.sit_u[i_w]));
  }

  for ( i_w = 0; i_w < src_u->reg_u.len_w; ++i_w ) {
    u3j_rite_merge(&(dst_u->reg_u.rit_u[i_w]), &(src_u->reg_u.rit_u[i_w]));
  }
}
