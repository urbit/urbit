...
_ci_slab_size(c3_g met_g, c3_d len_d)
{
  ...
  if ( (wor_w != wor_d) || (len_d != (bit_d >> met_g)) ) {
...
u3i_chub(c3_d dat_d)
{
  if ( c3y == u3a_is_cat(dat_d) ) {
    ...
    c3_w dat_w[2] = {dat_d & 0xffffffffULL, dat_d >> 32};
...
u3i_tape(const c3_c* txt_c)
{
  if ( !*txt_c ) {
    ...
    return u3i_cell(*txt_c, u3i_tape(txt_c + 1));
}
  ...
  if ( c3n == u3du(big) ) {
    ...
    switch ( axe_l ) {
      ...
      case 3:
        ...
        break;
      default: {
        u3_noun* tar = (2 == u3x_cap(axe_l)) ? &(cel_u->hed) : &(cel_u->tel);
...
_mutate(u3_noun big, u3_noun axe, u3_noun som)
{
  if ( c3y == u3a_is_cat(axe) ) {
    ...
    u3a_cell* cel_u = (void*)u3a_to_ptr(big);
    u3_noun   mor   = u3qc_mas(axe);
    u3_noun*  tar   = (2 == u3qc_cap(axe)) ? &(cel_u->hed) : &(cel_u->tel);
...
static c3_w
_molt_cut(c3_w len_w, struct _molt_pair* pms_m)
{
