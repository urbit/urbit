...
_ce_patch_save_page(u3_ce_patch* pat_u, c3_w pag_w, c3_w pgc_w)
{
  ...
  if ( u3P.dit_w[blk_w] & (1 << bit_w) ) {
    ...
    pat_u->con_u->mem_u[pgc_w].mug_w = u3r_mug_words(mem_w, (1 << u3a_page));
 ...
 */
static void
_ce_patch_junk_page(u3_ce_patch* pat_u, c3_w pag_w)
{
  c3_w blk_w = (pag_w >> 5);
  c3_w bit_w = (pag_w & 31);
 ...
 */
static void
_ce_image_blit(u3e_image* img_u, c3_w* ptr_w, c3_ws stp_ws)
{
  c3_w i_w;
...
_ce_image_fine(u3e_image* img_u, c3_w* ptr_w, c3_ws stp_ws)
{
  ...
  for ( i_w = 0; i_w < img_u->pgs_w; i_w++ ) {
    ...
    if ( mem_w != fil_w ) {
      ...
      fprintf(stderr,
              "mismatch: page %d, mem_w %x, fil_w %x, K %x\r\n",
              pag_w,
              mem_w,
              fil_w,
              u3K.mug_w[pag_w]);
...
#ifdef U3_SNAPSHOT_VALIDATION
  {
    _ce_image_fine(&u3P.nor_u, u3_Loom, (1 << u3a_page));
...
u3e_live(c3_o nuu_o, c3_c* dir_c)
{
...
#endif
  {
         ...
         || (c3n == _ce_image_open(&u3P.sou_u)) )
    {
       ...
       */
      {
        _ce_image_blit(&u3P.nor_u, u3_Loom, (1 << u3a_page));
