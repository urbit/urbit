...
_ptty_get_type(int fd)
{
  ...
  if ( wcsstr(name, L"\\ConEmu")
       || (wcsstr(name, L"msys-") || wcsstr(name, L"cygwin-"))
            && wcsstr(name, L"-pty") )
...
u3_ptty_init(uv_loop_t* lup_u, const c3_c** err_c)
{
  ...
  if ( pip_l == FILE_TYPE_CHAR ) {
         ...
         && 0 == (e = uv_pipe_open(&uty_u->pop_u.pip_u, 1)) )
    {
      fprintf(
        stderr,
        "vere: running interactive in a terminal emulator is experimental\r\n"
        "      use -t to disable interactivity or use native Windows "
        "console\r\n");
