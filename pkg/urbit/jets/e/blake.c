...
static u3_atom
_cqe_blake(u3_atom wid, u3_atom dat, u3_atom wik, u3_atom dak, u3_atom out)
{
  c3_w wid_w;
  if ( !u3r_word_fit(&wid_w, wid) ) {
    ...
    c3_w    wik_w = c3_min(wik, 64), out_w = c3_max(1, c3_min(out, 64));
...
u3we_blake(u3_noun cor)
{
  ...
  if ( c3n
         == u3r_mean(cor, u3x_sam_2, &msg, u3x_sam_6, &key, u3x_sam_7, &out, 0)
       || u3r_cell(msg, &wid, &dat) || u3ud(wid) || u3ud(dat)
       || u3r_cell(key, &wik, &dak) || u3ud(wik) || u3ud(dak) || u3ud(out) )
  {
