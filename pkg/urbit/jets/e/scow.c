...
_print_da(u3_noun cor, u3_atom raw_da)
{
  ...
  if ( f != 0 ) {
    ...
    for ( u3_noun cur = f_list; _(u3a_is_cell(cur)); cur = u3t(cur) ) {
  ...
  if ( f != 0 || hour != 0 || min != 0 || sec != 0 ) {
    if ( !_(u3a_is_cat(hour)) || !_(u3a_is_cat(min)) || !_(u3a_is_cat(sec)) ) {
  ...
  // though.
  if ( !_(u3a_is_cat(day)) || day > 99 || !_(u3a_is_cat(month)) || month > 99
       || !_(u3a_is_cat(year)) )
  {
    // Input is weird, fallback to nock.
    u3z(yod);
...
u3we_scow(u3_noun cor)
{
  ...
  if ( c3n == u3r_mean(cor, u3x_sam_2, &mod, u3x_sam_3, &atom, 0) ) {
    return u3m_bail(c3__exit);
  }
