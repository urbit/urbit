...
_lily_hike_belt_root(u3_noun pac)
{
  if ( (u3_nul == pac) ) {
    ...
    if ( c3y == u3r_sing(1, axis) ) {
      return u3nc(u3k(tool), list_tool);
    }
    else
      return list_tool;
  }
...
_lily_hike_belt_l(u3_noun pac)
{
  if ( (u3_nul == pac) ) {
    ...
    {
      if ( (1 != axis) && (c3y == u3r_sing(2, u3qc_cap(axis))) ) {
        u3_atom axis_tap = u3qc_mas(axis);

        return u3nc(u3nc(u3k(axis_tap), u3k(tool)), belt_l);
      }
      else
        return belt_l;
    }
...
_lily_hike_belt_r(u3_noun pac)
{
  if ( (u3_nul == pac) ) {
    ...
    {
      if ( (1 != axis) && (c3y == u3r_sing(3, u3qc_cap(axis))) ) {
        u3_atom axis_tap = u3qc_mas(axis);

        return u3nc(u3nc(u3k(axis_tap), u3k(tool)), belt_r);
      }
      else
        return belt_r;
    }
 ...
 */
u3_noun
u3qf_hike(u3_noun axe, u3_noun pac)
{
  c3_assert(0);
  if ( (u3_nul == pac) ) {
