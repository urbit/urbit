...
_bsw_atom(ur_root_t* r, ur_nref ref, ur_bsw_t* bsw, uint64_t len)
{
  switch ( ur_nref_tag(ref) ) {
    ...
    case ur_direct:
      return ur_bsw_atom64(bsw, len, ref);
...
static uint64_t
_jam(ur_jam_t* j, ur_nref ref, uint64_t* len, uint8_t** byt)
{
...
{
  return ur_jam_init_with(r,
                          ur_fib11,
                          ur_fib12, //  dict sizes
                          ur_fib10,
                          ur_fib11); //  stack sizes
}
...
static ur_cue_res_e
_cue(ur_cue_t* c, uint64_t len, const uint8_t* byt, ur_nref* out)
{
  ur_bsr_t      bsr  = {0};
  ur_root_t*    r    = c->r;
  _cue_stack_t* s    = &c->s;
...
static inline ur_cue_res_e
_cue_test_next(_cue_test_stack_t* s, ur_bsr_t* bsr, ur_dict_t* dict)
{
  while ( 1 ) {
    uint64_t     len, bits = bsr->bits;
    ur_cue_tag_e tag;
    ...
    switch ( tag ) {
      ...
      case ur_jam_back: {
        if ( ur_cue_good != (res = ur_bsr_rub_len(bsr, &len)) ) {
          ...
          return ur_dict_get((ur_root_t*)0, dict, bak) ? ur_cue_good
                                                       : ur_cue_back;
        }
      }
...
static ur_cue_res_e
_cue_test(ur_cue_test_t* t, uint64_t len, const uint8_t* byt)
{
  ur_bsr_t           bsr  = {0};
...
{
  return ur_cue_test_init_with(ur_fib11,
                               ur_fib12, //  dict sizes
                               ur_fib10,
                               ur_fib11); //  stack sizes
}
