...
ur_nref_mug(ur_root_t* r, ur_nref ref)
{
  switch ( ur_nref_tag(ref) ) {
    ...
    case ur_direct:
      return ur_mug64(ref);
    case ur_iatom:
      return r->atoms.mugs[ur_nref_idx(ref)];
    case ur_icell:
      return r->cells.mugs[ur_nref_idx(ref)];
  }
}
...
ur_atoms_grow(ur_atoms_t* atoms)
{
  ...
  atoms->bytes = _oom("atoms_grow",
                      malloc(next
                             * (sizeof(*atoms->bytes) + sizeof(*atoms->lens)
                                + sizeof(*atoms->mugs))));
...
ur_cells_grow(ur_cells_t* cells)
{
  ...
  cells->heads = _oom("cells_grow",
                      malloc(next
                             * (sizeof(*cells->heads) + sizeof(*cells->heads)
                                + sizeof(*cells->mugs))));
...
ur_met(ur_root_t* r, uint8_t bloq, ur_nref ref)
{
  ...
  switch ( bloq ) {
    case 0:
      return m_bit;
    case 1:
      return ur_bloq_up1(m_bit);
    case 2:
      return ur_bloq_up2(m_bit);
    case 3:
      return ur_bloq_up3(m_bit);
...
ur_coin_bytes_unsafe(ur_root_t* r, uint64_t len, uint8_t* byt)
{
  ...
  while ( 1 ) {
    ...
    for ( i = 0; i < b_fill; i++ ) {
      ...
      if ( (t_len == len) && (0 == memcmp(t_byt, byt, len)) ) {
        return tom;
      }
    }
...
ur_cons(ur_root_t* r, ur_nref hed, ur_nref tal)
{
  ...
  ur_mug      mug   = ur_mug_both(ur_nref_mug(r, hed), ur_nref_mug(r, tal));
  ...
  while ( 1 ) {
    ...
    for ( i = 0; i < b_fill; i++ ) {
      ...
      if ( (hed == ur_head(r, cel)) && (tal == ur_tail(r, cel)) ) {
        return cel;
      }
    }
...
_atoms_info(FILE* f, ur_atoms_t* atoms)
{
  ...
  uint64_t refs
    = size
      * (sizeof(*atoms->bytes) + sizeof(*atoms->lens) + sizeof(*atoms->mugs));
  uint64_t i, data = 0;
...
_cells_info(FILE* f, ur_cells_t* cells)
{
  ...
  uint64_t refs
    = size
      * (sizeof(*cells->heads) + sizeof(*cells->heads) + sizeof(*cells->mugs));
