  ...
  if ( !left ) {
    ...
    if ( len < rest ) {
      ...
      switch ( len_byt ) {
        ...
        case 4: {
          l = (uint32_t)b[0] ^ (uint32_t)b[1] << 8 ^ (uint32_t)b[2] << 16
              ^ (uint32_t)b[3] << 24;
        } break;
...
ur_bsr64_any(ur_bsr_t* bsr, uint8_t len)
{
  ...
  if ( !left ) {
    ...
    if ( len < rest ) {
      ...
      switch ( len_byt ) {
        case 8: {
          l = (uint64_t)b[0] ^ (uint64_t)b[1] << 8 ^ (uint64_t)b[2] << 16
...
{
  return ((8 > bsw->off) && ((bsw->fill << 3) + bsw->off == bsw->bits));
}
...
{
  if ( (7 == bsw->off) && ((1 + bsw->fill) == bsw->size) ) {
