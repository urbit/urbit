...
#define u3a_boxto(box_v) ((void*)(((c3_w*)(void*)(box_v)) + c3_wiseof(u3a_box)))
#define u3a_botox(tox_v) \
  ((struct _u3a_box*)(void*)(((c3_w*)(void*)(tox_v)) - c3_wiseof(u3a_box)))
/* Inside a noun.
 */
...
#define u3a_h(som)                                           \
  (_(u3a_is_cell(som)) ? (((u3a_cell*)u3a_to_ptr(som))->hed) \
                       : u3m_bail(c3__exit))

#define u3a_t(som)                                           \
  (_(u3a_is_cell(som)) ? (((u3a_cell*)u3a_to_ptr(som))->tel) \
                       : u3m_bail(c3__exit))
 ...
 */
#define u3a_open(r)                                       \
  ((c3y == u3a_is_north(r)) ? (c3_w)(r->cap_p - r->hat_p) \
...
*/
#define u3a_full(r)                                       \
  ((c3y == u3a_is_north(r)) ? (c3_w)(r->mat_p - r->rut_p) \
                            : (c3_w)(r->rut_p - r->mat_p))

/* u3a_heap(): words of heap in [r]
 */
#define u3a_heap(r)                                       \
  ((c3y == u3a_is_north(r)) ? (c3_w)(r->hat_p - r->rut_p) \
                            : (c3_w)(r->rut_p - r->hat_p))

/* u3a_temp(): words of stack in [r]
 */
#define u3a_temp(r)                                       \
  ((c3y == u3a_is_north(r)) ? (c3_w)(r->mat_p - r->cap_p) \
                            : (c3_w)(r->cap_p - r->mat_p))

#define u3a_north_is_senior(r, dog) \
  __((u3a_to_off(dog) < r->rut_p) || (u3a_to_off(dog) >= r->mat_p))

#define u3a_north_is_junior(r, dog) \
  __((u3a_to_off(dog) >= r->cap_p) && (u3a_to_off(dog) < r->mat_p))

#define u3a_north_is_normal(r, dog) \
  c3a(!(u3a_north_is_senior(r, dog)), !(u3a_north_is_junior(r, dog)))

#define u3a_south_is_senior(r, dog) \
  __((u3a_to_off(dog) < r->mat_p) || (u3a_to_off(dog) >= r->rut_p))

#define u3a_south_is_junior(r, dog) \
  __((u3a_to_off(dog) < r->cap_p) && (u3a_to_off(dog) >= r->mat_p))

#define u3a_south_is_normal(r, dog) \
  c3a(!(u3a_south_is_senior(r, dog)), !(u3a_south_is_junior(r, dog)))

#define u3a_is_junior(r, som)                         \
  (_(u3a_is_cat(som))   ? c3n                         \
   : _(u3a_is_north(r)) ? u3a_north_is_junior(r, som) \
                        : u3a_south_is_junior(r, som))

#define u3a_is_senior(r, som)                         \
  (_(u3a_is_cat(som))   ? c3y                         \
   : _(u3a_is_north(r)) ? u3a_north_is_senior(r, som) \
                        : u3a_south_is_senior(r, som))

#define u3a_is_mutable(r, som)                      \
  (_(u3a_is_atom(som))                        ? c3n \
   : _(u3a_is_senior(r, som))                 ? c3n \
   : _(u3a_is_junior(r, som))                 ? c3n \
   : (u3a_botox(u3a_to_ptr(som))->use_w == 1) ? c3y \
                                              : c3n)
