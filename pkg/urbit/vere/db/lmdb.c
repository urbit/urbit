             ...
             c3_o (*read_f)(void*, c3_d, size_t, void*))
{
...

  {
    ...
    if ( (ret_w = mdb_cursor_get(cur_u, &key_u, &val_u, MDB_SET_KEY)) ) {
      mdb_logerror(stderr,
                   ret_w,
                   "lmdb: read: initial cursor_get failed at %" PRIu64,
                   eve_d);
      mdb_cursor_close(cur_u);
      //  XX confirm
      //
    ...
    //
    {
      ...
      for ( i_d = 0; (ret_w != MDB_NOTFOUND) && (i_d < len_d); ++i_d ) {
        ...
        if ( *(c3_d*)key_u.mv_data != cur_d ) {
          fprintf(stderr,
                  "lmdb: read gap: expected %" PRIu64 ", received %" PRIu64
                  "\r\n",
                  cur_d,
                  *(c3_d*)key_u.mv_data);
          ret_o = c3n;
 ...
 */
c3_o
u3_lmdb_save_meta(MDB_env* env_u, const c3_c* key_c, size_t val_i, void* val_p)
{
  MDB_txn* txn_u;
  MDB_dbi  mdb_u;
  c3_w     ret_w;
