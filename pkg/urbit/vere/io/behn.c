...
_behn_wake_bail(u3_ovum* egg_u, u3_noun lud)
{
  ...
  if ( (2 > egg_u->try_w) && (c3n == _behn_bail_dire(lud)) ) {
    u3z(lud);
...
_behn_time_cb(uv_timer_t* tim_u)
{
  ...
  //
  {
    ...
    u3_auto_peer(u3_auto_plan(&teh_u->car_u, u3_ovum_init(0, c3__b, wir, cad)),
                 0,
                 0,
                 _behn_wake_bail);
  }
...
_behn_ef_doze(u3_behn* teh_u, u3_noun wen)
{
  ...
  if ( (u3_nul != wen) && (c3y == u3du(wen)) && (c3y == u3ud(u3t(wen))) ) {
...
_behn_io_talk(u3_auto* car_u)
{
  ...
  //  XX remove [sev_l]
  //
  u3_noun wir = u3nt(c3__behn, u3dc("scot", c3__uv, teh_u->sev_l), u3_nul);
  u3_noun cad = u3nc(c3__born, u3_nul);

  u3_auto_peer(u3_auto_plan(car_u, u3_ovum_init(0, c3__b, wir, cad)),
               0,
               _behn_born_news,
               _behn_born_bail);
}
