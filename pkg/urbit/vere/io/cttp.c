...
_cttp_heds_to_noun(h2o_header_t* hed_u, c3_d hed_d)
{
  ...
  while ( 0 < dex_d ) {
    deh_u = hed_u[--dex_d];
    hed   = u3nc(
      u3nc(_cttp_vec_to_atom(*deh_u.name), _cttp_vec_to_atom(deh_u.value)),
      hed);
  }
...
_cttp_creq_fire(u3_creq* ceq_u)
{
  ...
  if ( !ceq_u->bod_u ) {
    ...
    c3_c len_c[41];
    c3_w len_w
      = snprintf(len_c, 40, "Content-Length: %u\r\n\r\n", ceq_u->bod_u->len_w);
...
_cttp_creq_respond(u3_creq* ceq_u)
{
  ...
  _cttp_http_client_receive(
    ...
    res_u->sas_w,
    res_u->hed,
    (!res_u->bod_u) ? u3_nul : u3nc(u3_nul, _cttp_bods_to_octs(res_u->bod_u)));
...
_cttp_creq_on_head(h2o_http1client_t* cli_u,
                   const c3_c*        err_c,
                   c3_i               ver_i,
                   c3_i               sas_i,
                   h2o_iovec_t        sas_u,
...
_cttp_creq_on_connect(h2o_http1client_t* cli_u,
                      const c3_c*        err_c,
                      h2o_iovec_t**      vec_u,
                      size_t*            vec_i,
                      c3_i*              hed_i)
...
_cttp_creq_connect(u3_creq* ceq_u)
{
  ...
  //
  {
    ...
    c3_s        por_s = (ceq_u->por_s) ? ceq_u->por_s : (tls_t) ? 443 : 80;

    h2o_http1client_connect(&ceq_u->cli_u,
                            ceq_u,
                            &ceq_u->ctp_u->ctx_u,
                            ipf_u,
                            por_s,
                            tls_t,
                            _cttp_creq_on_connect);
  }
                      ...
                      struct addrinfo*  aif_u)
{
  ...
  if ( u3_csat_quit == ceq_u->sat_e ) {
    ...
    ceq_u->ipf_w
      = ntohl(((struct sockaddr_in*)aif_u->ai_addr)->sin_addr.s_addr);
    ceq_u->ipf_c = _cttp_creq_ip(ceq_u->ipf_w);
