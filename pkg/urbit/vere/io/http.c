...
_http_vec_to_meth(h2o_iovec_t vec_u)
{
  return (0 == strncmp(vec_u.base, "GET", vec_u.len))    ? u3i_string("GET")
         ...
         : (0 == strncmp(vec_u.base, "CONNECT", vec_u.len))
           ? u3i_string("CONNECT")
         : (0 == strncmp(vec_u.base, "DELETE", vec_u.len))
           ? u3i_string("DELETE")
         : (0 == strncmp(vec_u.base, "OPTIONS", vec_u.len))
           ? u3i_string("OPTIONS")
         : (0 == strncmp(vec_u.base, "TRACE", vec_u.len))
           ? u3i_string("TRACE")
           :
           // TODO ??
...
_http_heds_to_noun(h2o_header_t* hed_u, c3_d hed_d)
{
  ...
  while ( 0 < dex_d ) {
    deh_u = hed_u[--dex_d];
    hed   = u3nc(
      u3nc(_http_vec_to_atom(*deh_u.name), _http_vec_to_atom(deh_u.value)),
      hed);
  }
...
{
  u3_hreq* req_u
    = h2o_mem_alloc_shared(&rec_u->pool, sizeof(*req_u), _http_req_done);
  req_u->rec_u = rec_u;
...
{
  u3_hreq* req_u
    = h2o_mem_alloc_shared(&rec_u->pool, sizeof(*req_u), _http_seq_done);
  req_u->rec_u = rec_u;
                    ...
                    u3_noun  complete)
{
  ...
  u3_hgen* gen_u
    = h2o_mem_alloc_shared(&rec_u->pool, sizeof(*gen_u), _http_hgen_dispose);
  gen_u->neg_u = (h2o_generator_t){_http_hgen_proceed, _http_hgen_stop};
  gen_u->red   = c3y;
  gen_u->dun   = complete;
  gen_u->bod_u = (u3_nul == data) ? 0 : _cttp_bod_from_octs(u3k(u3t(data)));
  gen_u->nud_u = 0;
  gen_u->hed_u = deh_u;
  gen_u->req_u = req_u;
  ...
  //
  if ( 1 == has_len_i ) {
    rec_u->res.content_length = (0 == gen_u->bod_u) ? 0 : gen_u->bod_u->len_w;
  }
...
_http_rec_to_httq(h2o_req_t* rec_u)
{
  ...
  u3_noun hed = _http_heds_to_noun(rec_u->headers.entries, rec_u->headers.size);

  // restore host header
  hed
    = u3nc(u3nc(u3i_string("host"), _http_vec_to_atom(rec_u->authority)), hed);
 ...
 */
static u3_hreq*
_http_req_prepare(h2o_req_t* rec_u, u3_hreq* (*new_f)(u3_hcon*, h2o_req_t*))
{
  h2o_uv_sock* suv_u
    = (h2o_uv_sock*)rec_u->conn->callbacks->get_socket(rec_u->conn);
...
_http_seq_continue(void* vod_p, u3_noun nun)
{
  ...
  if ( c3y == aut ) {
    ...
    u3_noun  hed
      = u3nl(u3nc(u3i_string("Content-Type"), u3i_string("text/event-stream")),
...
_http_seq_accept(h2o_handler_t* han_u, h2o_req_t* rec_u)
{
  ...
  //
    ...
    u3_noun pax = u3nq(u3i_string("authenticated"),
                       ...
                       u3dc("scot", 't', coo),
                       u3_nul);
    u3_pier_peek_last(hon_u->htp_u->htd_u->car_u.pir_u,
                      u3_nul,
                      c3__ex,
                      u3_nul,
                      pax,
...
_http_h2o_context_dispose(h2o_context_t* ctx)
{
  ...
  h2o_multithread_unregister_receiver(ctx->queue,
                                      &ctx->receivers.hostinfo_getaddr);
...
_http_serv_accept(u3_http* htp_u)
{
  ...
  if ( 0
       != (sas_i = uv_accept((uv_stream_t*)&htp_u->wax_u,
  ...
  hon_u->sok_u
    = h2o_uv_socket_create((uv_stream_t*)&hon_u->wax_u, _http_conn_free);
  ...
  hon_u->ipf_w
    = (adr_u.sin_family != AF_INET) ? 0 : ntohl(adr_u.sin_addr.s_addr);
}
...
_http_serv_init_h2o(SSL_CTX* tls_u, c3_o log, c3_o red)
{
  ...
  h2o_u->fig_u.server_name
    = h2o_iovec_init(H2O_STRLIT("urbit/vere-" URBIT_VERSION));
  ...
  h2o_u->han_u
    = h2o_create_handler(&h2o_u->hos_u->fallback_path, sizeof(*h2o_u->han_u));
  if ( c3y == red ) {
  ...
  h2o_pathconf_t* pac_u
    = h2o_config_register_path(h2o_u->hos_u, "/~_~/slog", 0);
...
_http_serv_start(u3_http* htp_u)
{
  ...
  adr_u.sin_addr.s_addr
    = (c3y == htp_u->lop) ? htonl(INADDR_LOOPBACK) : INADDR_ANY;
  ...
  while ( 1 ) {
    ...
    if ( 0
           != (sas_i
               = uv_tcp_bind(&htp_u->wax_u, (const struct sockaddr*)&adr_u, 0))
         || 0
              != (sas_i = uv_listen((uv_stream_t*)&htp_u->wax_u,
                                    TCP_BACKLOG,
                                    _http_serv_listen_cb)) )
    {
...
static u3_hreq*
_http_search_req(u3_httd* htd_u, c3_l sev_l, c3_l coq_l, c3_l seq_l)
{
  u3_http* htp_u;
  u3_hcon* hon_u;
  u3_hreq* req_u;
...
u3_http_ef_form(u3_httd* htd_u, u3_noun fig)
{
  ...
  if ( (c3n == u3r_qual(fig, &sec, &pro, &log, &red)) ||
       ...
       !(u3_nul == sec
         || (c3y == u3du(sec) && c3y == u3du(u3t(sec)) && u3_nul == u3h(sec)))
       ||
       // confirm valid flags ("loobeans")
       !(c3y == pro || c3n == pro) || !(c3y == log || c3n == log)
       || !(c3y == red || c3n == red) )
  {
...
_http_stream_slog(void* vop_p, c3_w pri_w, u3_noun tan)
{
  ...
  if ( 0 != seq_u ) {
    ...
    if ( c3y == u3a_is_atom(tan) ) {
      u3_noun lin
        = u3i_list(u3i_string("data:"), u3k(tan), c3_s2('\n', '\n'), u3_none);
...
_http_seq_heartbeat_cb(uv_timer_t* tim_u)
{
  ...
  uv_timer_start(htd_u->fig_u.sit_u,
                 _http_seq_heartbeat_cb,
                 HEARTBEAT_TIMEOUT,
                 0);
}
 ...
 */
static u3_noun
_reck_mole(u3_noun fot, u3_noun san, c3_d* ato_d)
{
  u3_noun uco = u3dc("slaw", fot, san);
  u3_noun p_uco, q_uco;

  if ( (c3n == u3r_cell(uco, &p_uco, &q_uco)) || (u3_nul != p_uco) ) {
  ...
  //
  {
    ...
    if ( u3_nul == t_pud ) {
      ...
      if ( u3_nul == tt_pud ) {
        ...
        if ( (c3n == u3r_cell(tt_pud, &r_pud, &s_pud)) || (u3_nul != s_pud)
