 ...
 */
static void
_ames_alloc(uv_handle_t* had_u, size_t len_i, uv_buf_t* buf)
{
...
{
  c3_w hed_w = buf_y[0] | (buf_y[1] << 8) | (buf_y[2] << 16) | (buf_y[3] << 24);
...
{
  return (c3_d)byt_y[0] | (c3_d)byt_y[1] << 8 | (c3_d)byt_y[2] << 16
         | (c3_d)byt_y[3] << 24 | (c3_d)byt_y[4] << 32 | (c3_d)byt_y[5] << 40
         | (c3_d)byt_y[6] << 48 | (c3_d)byt_y[7] << 56;
}
 ...
 */
static c3_o
_ames_sift_body(u3_head* hed_u, u3_body* bod_u, c3_w len_w, c3_y* bod_y)
{
...
{
  c3_w hed_w = ((hed_u->sim_o & 0x1) << 3) ^ ((hed_u->ver_y & 0x7) << 4)
               ^ ((hed_u->sac_y & 0x3) << 7) ^ ((hed_u->rac_y & 0x3) << 9)
 ...
 */
static c3_w
_ames_etch_pack(u3_head* hed_u, u3_body* bod_u, c3_y** out_y)
{
...
_ames_serialize_packet(u3_panc* pac_u, c3_o dop_o)
{
  ...
  //
  if ( c3y == dop_o && c3n == pac_u->hed_u.rel_o
       && !((256 > pac_u->bod_u.sen_d[0]) && (0 == pac_u->bod_u.sen_d[1])) )
  {
  ...
  //
  {
    u3_noun pac;
    c3_y*   pac_y;
    c3_w    len_w = _ames_etch_pack(&pac_u->hed_u, &pac_u->bod_u, &pac_y);
 ...
 */
static void
_ames_czar_cb(uv_getaddrinfo_t* adr_u, c3_i sas_i, struct addrinfo* aif_u)
{
  {
...
_ames_hear_bail(u3_ovum* egg_u, u3_noun lud)
{
  ...
  if ( (u3C.wag_w & u3o_verbose) || (0 == (sam_u->sat_u.fal_d % 1000)) ) {
    _ames_punt_goof(lud);
 ...
 */
static void
_ames_put_packet(u3_ames* sam_u, u3_noun msg, u3_lane lan_u)
{
  u3_noun wir = u3nc(c3__ames, u3_nul);
  u3_noun cad = u3nt(c3__hear, u3nc(c3n, u3_ames_encode_lane(lan_u)), msg);

  u3_auto_peer(u3_auto_plan(&sam_u->car_u, u3_ovum_init(0, c3__a, wir, cad)),
               0,
               0,
               _ames_hear_bail);
                  ...
                  c3_y*    hun_y)
{
  ...
  //
  if ( (256 > bod_u->rec_d[0]) && (0 == bod_u->rec_d[1]) ) {
...
{
  if ( bod_u->sen_d[1] == 0
       && (bod_u->sen_d[0] == 0x743a17a6 || bod_u->sen_d[0] == 0xea99acb6
 ...
 */
static void
_ames_hear(u3_ames* sam_u, u3_lane* lan_u, c3_w len_w, c3_y* hun_y)
{
  u3_head hed_u;
  u3_body bod_u;
  ...
  //
  if ( (4 > len_w) || (c3n == _ames_sift_head(&hed_u, hun_y)) ) {
    sam_u->sat_u.hed_d++;
  ...
  //
  if ( (c3y == sam_u->fig_u.fit_o) && (sam_u->ver_y != hed_u.ver_y) ) {
    sam_u->sat_u.vet_d++;
...
_ames_io_start(u3_ames* sam_u)
{
  ...
  //  Bind and stuff.
  {
    ...
    memset(&add_u, 0, sizeof(add_u));
    add_u.sin_family = AF_INET;
    add_u.sin_addr.s_addr
      = _(u3_Host.ops_u.net) ? htonl(INADDR_ANY) : htonl(INADDR_LOOPBACK);
         ...
         != 0 )
    {
      ...
      if ( (c3__czar == rac) && (UV_EADDRINUSE == ret_i) ) {
...
_ames_prot_scry_cb(void* vod_p, u3_noun nun)
{
  ...
  else if ( (c3n == u3a_is_cat(ver)) || (7 < ver) ) {
...
_ames_io_talk(u3_auto* car_u)
{
  ...
  //
  {
    //  XX remove [sev_l]
    //
    u3_noun wir = u3nt(c3__newt, u3dc("scot", c3__uv, sam_u->sev_l), u3_nul);
