                ...
                u3_noun    act)
{
  ...
  u3_fact* tac_u = 0;
  // u3_fact* tac_u = u3_fact_init(god_u->eve_d, mug_l, job);
  // god_u->mug_l   = mug_l;
  // god_u->eve_d   = eve_d;

  u3_gift* gif_u = u3_gift_init(god_u->eve_d, act);
...
_lord_writ_make(u3_lord* god_u, u3_writ* wit_u)
{
  ...
  switch ( wit_u->typ_e ) {
    ...
    case u3_writ_work: {
      // u3m_p("poke out", wit_u->wok_u.job);
    ...
    case u3_writ_save: {
      msg = u3nt(c3__sync, c3__save, u3_nul);
    } break;
