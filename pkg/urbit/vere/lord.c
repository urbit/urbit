...
+$  writ
  $%  $:  %live
          $%  [%cram eve=@]
              ...
              [%pack ~]
      ==  ==
      [%peek mil=@ sam=*]  :: gang (each path $%([%once @tas @tas path] [beam
@tas beam]))
 ...
 */
static void
_lord_stop_cb(void* ptr_v, ssize_t err_i, const c3_c* err_c)
{
  u3_lord* god_u = ptr_v;
 ...
 */
static void
_lord_bail_noop(void* ptr_v, ssize_t err_i, const c3_c* err_c)
{}
...
_lord_plea_peek_done(u3_lord* god_u, u3_peek* pek_u, u3_noun rep)
{
  //  XX review
  //
  if ( (u3_pico_once == pek_u->typ_e) && (u3_nul != rep) ) {
    u3_noun dat;
...
_lord_plea_play_bail(u3_lord* god_u, u3_info fon_u, u3_noun dat)
{
  ...
  if ( (c3n == u3r_trel(dat, &eve, &mug, &dud))
       || (c3n == u3r_safe_chub(eve, &eve_d))
       || (c3n == u3r_safe_word(mug, &mug_l)) || (c3n == u3a_is_cell(dud)) )
  {
  ...
  god_u->cb_u.play_bail_f(god_u->cb_u.ptr_v, fon_u, mug_l, eve_d, u3k(dud));
...
_lord_plea_work_swap(u3_lord* god_u, u3_ovum* egg_u, u3_noun dat)
{
  ...
  if ( (c3n == u3r_qual(dat, &eve, &mug, &job, &act))
       || (c3n == u3r_safe_chub(eve, &eve_d))
       || (c3n == u3r_safe_word(mug, &mug_l)) || (c3n == u3a_is_cell(job)) )
  {
    u3z(job);
    u3_ovum_free(egg_u);
 ...
 */
static void
_lord_plea_work_done(u3_lord* god_u, u3_ovum* egg_u, u3_noun job, u3_noun dat)
{
  u3_noun eve, mug, act;
  c3_d    eve_d;
  c3_l    mug_l;
...
u3_lord_work(u3_lord* god_u, u3_ovum* egg_u, u3_noun job)
{
  ...
  //
  if ( c3n == god_u->pin_o ) {
    god_u->cb_u.spin_f(god_u->cb_u.ptr_v, egg_u->pin_u.lab, egg_u->pin_u.del_o);
    god_u->pin_o = c3y;
  }
 ...
 */
static void
_lord_on_serf_err_cb(uv_stream_t* pyp_u, ssize_t siz_i, const uv_buf_t* buf_u)
{
  if ( siz_i >= 0 ) {
 ...
 */
static void
_lord_on_serf_exit(uv_process_t* req_u, c3_ds sas_i, c3_i sig_i)
{
  u3_lord* god_u = (void*)req_u;

  if ( !god_u->ext_u || !(u3_writ_exit == god_u->ext_u->typ_e) ) {
    fprintf(stderr,
            "pier: work exit: status %" PRId64 ", signal %d\r\n",
            sas_i,
            sig_i);
    _lord_bail(god_u);
  }
 ...
 */
static void
_lord_on_serf_bail(void* ptr_v, ssize_t err_i, const c3_c* err_c)
{
  u3_lord* god_u = ptr_v;
...
u3_lord_init(c3_c* pax_c, c3_w wag_w, c3_d key_d[4], u3_lord_cb cb_u)
{
  ...
  //
  {
    ...
    uv_read_start((uv_stream_t*)&god_u->err_u,
                  _lord_serf_err_alloc,
                  _lord_on_serf_err_cb);
  }
