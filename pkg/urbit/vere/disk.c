...
_disk_commit_done(struct _cd_save* req_u)
{
  ...
  if ( c3n == ret_o ) {
    ...
    if ( 1ULL == len_d ) {
      ...
      fprintf(stderr,
              "disk: (%" PRIu64 "-%" PRIu64 "): commit: failed\r\n",
              eve_d,
              eve_d + (len_d - 1ULL));
    }
#endif
    ...
    if ( 1ULL == len_d ) {
      ...
      fprintf(stderr,
              "disk: (%" PRIu64 "-%" PRIu64 "): commit: complete\r\n",
              eve_d,
              eve_d + (len_d - 1ULL));
    }
#endif
...
_disk_commit_start(struct _cd_save* req_u)
{
  ...
  //
  uv_queue_work(u3L, &log_u->ted_u, _disk_commit_cb, _disk_commit_after_cb);
}
...
_disk_commit(u3_disk* log_u)
{
  if ( (c3n == log_u->ted_o) && (log_u->sen_d > log_u->dun_d) ) {
    ...
    if ( 1ULL == len_d ) {
      ...
      fprintf(stderr,
              "disk: (%" PRIu64 "-%" PRIu64 "): commit: request\r\n",
              req_u->eve_d,
...
_disk_read_one_cb(void* ptr_v, c3_d eve_d, size_t val_i, void* val_p)
{
...

  {
    ...
    c3_y*   dat_y = val_p;
    c3_l    mug_l
      = dat_y[0] ^ (dat_y[1] << 8) ^ (dat_y[2] << 16) ^ (dat_y[3] << 24);
