...
shasByteStrings = (Dawn.shas testSalt testString) @?= expected
 where
  expected =
    cordToAtomBytes "0wfKW.mXzrj.c~IBb.lKd6k.2njoG.bRLcD.9eszA.gSSs8.mHRah"

shafByteStrings = (Dawn.shaf testSalt testString) @?= expected
  where expected = cordToAtomBytes "0w3h.Bg1Qh.ZZjoJ.23J~p.PHg-D"
...
cometShip = case Ob.parsePatp cometStr of
  ...
  where cometStr = "~radmes-dilsec-sovlup-lagwep--tonred-waldeb-tocseg-marzod"
