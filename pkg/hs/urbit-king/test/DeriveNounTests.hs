...
instance Arbitrary FooBar where
  arbitrary = oneof
    [ FooBarQueenAlice <$> arbitrary <*> arbitrary
    , FooBarBob <$> arbitrary
    , pure FooBarCharlie
    ]
...
instance Arbitrary BarZaz where
  arbitrary = oneof
    [ BZQueenAlice <$> arbitrary <*> arbitrary
    , BZBob <$> arbitrary
    , pure BZCharlie
    ]
...
instance Arbitrary ZazBaz where
  arbitrary = oneof
    [QueenAlice <$> arbitrary <*> arbitrary, Bob <$> arbitrary, pure Charlie]

instance (Arbitrary a, Arbitrary b) => Arbitrary (Poly a b) where
  arbitrary = oneof [PLeft <$> arbitrary, PRite <$> arbitrary]
...
nounEquiv
  :: (Eq a, Eq b, ToNoun a, ToNoun b, FromNoun a, FromNoun b)
  => (a -> b)
  -> a
  -> Bool
nounEquiv cvt x = and [Just x == throughNoun y, Just y == throughNoun x]
  where y = cvt x
...
recSanity x = toNoun x == byHand x
  where byHand (ThreeWords x y z) = toNoun (x, y, z)
...
barZazBaz = nounEquiv $ \case
  BZQueenAlice x y -> QueenAlice x y
  BZBob x          -> Bob x
  BZCharlie        -> Charlie

fooBarBaz :: FooBar -> Bool
fooBarBaz = nounEquiv $ \case
  FooBarQueenAlice x y -> QueenAlice x y
  FooBarBob x          -> Bob x
