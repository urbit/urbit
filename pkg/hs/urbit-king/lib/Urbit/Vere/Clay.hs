...
-}
foreachFileIn
  :: (MonadUnliftIO m) => FilePath -> (FilePath -> (ResourceT m) a) -> m [a]
foreachFileIn fp fun =
  runConduitRes
    $  (sourceDirectoryDeep False fp)
...
takeFilesystemSnapshot fp = do
  ...
  if not exists
    then pure M.empty
    else M.fromList <$> foreachFileIn fp getHashOfFile
...
-}
buildActionListFromDifferences
  :: FilePath -> Map FilePath Int -> RIO e [(FilePath, Maybe (Mime, Int))]
...
clay
  ...
  => KingId
  -> QueueEv
  -> ([Ev], RAcquire e (EffCb e SyncEf))
clay king enqueueEv = (initialEvents, runSync)
 where
  initialEvents =
  ...
  handleEffect cd = \case
    ...
    SyncEfDirk p desk -> do
      ...
      atomically $ enqueueEv $ EvBlip $ BlipEvSync $ SyncEvInto
        (Some (king, ()))
        desk
        False
        intoList

      atomically $ modifyTVar (cdMountPoints cd)
  ...
  -- in Unix.
  calculateActionHash
    :: FilePath -> (Path, Maybe Mime) -> (FilePath, Maybe (Mime, Int))
  ...
  performAction
    :: (Map FilePath Int) -> (FilePath, Maybe (Mime, Int)) -> RIO e ()
  ...
  actionsToInto
    :: FilePath -> (FilePath, Maybe (Mime, Int)) -> (Path, Maybe Mime)
