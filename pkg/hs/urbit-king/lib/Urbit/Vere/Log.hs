...
create dir id = do
 ...
 where
  createTables env = rwith (writeTxn env) $ \txn ->
    io
      $   (,,)
...
open dir = do
 ...
 where
  openTables env = rwith (writeTxn env) $ \txn ->
    io
      $   (,,)
...
getNumEvents env eventsTbl = rwith (readTxn env) $ \txn ->
...
clearEvents env eventsTbl = rwith (writeTxn env) $ \txn ->
  rwith (cursor txn eventsTbl) $ \cur ->
...
appendEvents log !events = do
 ...
 where
  ...
  doAppend = \kvs -> rwith (writeTxn $ env log) $ \txn ->
    for_ kvs $ \(k, v) -> do
...
trimEvents log start = do
  last <- lastEv log
  rwith (writeTxn $ env log) $ \txn -> for_ [start .. last] $ \eId ->
...
streamEvents
  :: HasLogFunc e => EventLog -> Word64 -> ConduitT () ByteString (RIO e) ()
...
readBatch log first = start
 where
  ...
  readRows count = withWordPtr first $ \pIdx ->
...
readRowsBatch
  ...
  -> Dbi
  -> Word64
  -> RIO e (V.Vector (Word64, ByteString))
 ...
 where
  readRows = do
    ...
    withWordPtr first $ \pIdx ->
      withKVPtrs' (MDB_val 8 (castPtr pIdx)) nullVal $ \pKey pVal ->
        rwith (readTxn env) $ \txn -> rwith (cursor txn dbi) $ \cur ->
