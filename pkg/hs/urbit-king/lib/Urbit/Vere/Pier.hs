...
pier
  :: forall e
   . (HasConfigDir e, HasLogFunc e, HasNetworkConfig e, HasPierConfig e)
...
drivers inst who isFake plan shutdownSTM termSys stderr =
 ...
 where
  ...
  initialEvents =
    mconcat [behnBorn, clayBorn, amesBorn, httpBorn, termBorn, irisBorn]
  runDrivers = do
    dNewt       <- runAmes
...
router waitFx Drivers {..} = mkRAcquire start cancel
 where
  start = async $ forever $ do
...
runCompute
  ...
  -> ((Job, FX) -> STM ())
  -> RAcquire e (Async ())
runCompute serf ss getEvent getSaveSignal getShutdownSignal showSpinner hideSpinner putResult
...
instance Exception PersistExn where
  displayException (BadEventId expected got) = unlines
    [ "Out-of-order event id send to persist thread."
...
runPersist log inpQ out = mkRAcquire runThread cancel
 where
  ...
  validateJobsAndGetBytes writs = do
    expect <- Log.nextEv log
    fmap fromList $ for (zip [expect ..] writs) $ \(expectedId, (j, fx)) -> do
      unless (expectedId == jobId j) $ throwIO (BadEventId expectedId (jobId j))
      case j of
        RunNok _                     -> error "This shouldn't happen here!"
        DoWork (Work eId mug wen ev) -> pure $ jamBS $ toNoun (mug, wen, ev)
