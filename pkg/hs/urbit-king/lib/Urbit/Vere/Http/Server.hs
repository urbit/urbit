...
reorgHttpEvent = \case
  ...
  Cancel ()             -> [RADone]
  Continue mBlk isDone ->
    toList (RABloc <$> mBlk) <> if isDone then [RADone] else []
...
removePortsFile pax = io (doesFileExist pax) >>= \case
...
reqEv sId reqId which addr req = case which of
  Loopback ->
    servEv $ HttpServerEvRequestLocal (sId, reqId, 1, ()) $ HttpServerReq
      False
      addr
      req
  _ -> servEv $ HttpServerEvRequest (sId, reqId, 1, ()) $ HttpServerReq
    (which == Secure)
    addr
    req
...
streamBlocks env init tmv = for_ init yieldÇunk >> go
 where
...
sendResponse cb tmv = do
  ...
  getResp tmv >>= \case
    ...
    RFull h f ->
      io
        $   cb
        $   W.responseLBS (hdrStatus h) (hdrHeaders h)
        $   fromStrict
        $   concat
        $   unOcts
        .   unFile
        <$> f
    RHead h i ->
      io $ cb $ W.responseSource (hdrStatus h) (hdrHeaders h) $ streamBlocks
        env
        i
        tmv
 where
...
app env sId liv plan which req respond =
  runRIO env $ rwith (liveReq liv) $ \(reqId, respVar) -> do
    body <- reqBody req
...
startServ isFake conf plan = do
  ...
  let loopOpts =
        W.defaultSettings
          & W.setPort (fromIntegral loopPort)
          & W.setHost "127.0.0.1"
          & W.setTimeout (5 * 60)
      httpOpts =
        W.defaultSettings & W.setHost "*" & W.setPort (fromIntegral httpPort)
      httpsOpts =
        W.defaultSettings & W.setHost "*" & W.setPort (fromIntegral httpsPort)

  env <- ask

  logDebug "Starting loopback server"
  loopTid <- async $ io $ W.runSettingsSocket loopOpts loopSock $ app env
                                                                      sId
                                                                      liv
                                                                      plan
                                                                      Loopback
  ...
  httpTid <- async $ io $ W.runSettingsSocket httpOpts httpSock $ app env
                                                                      sId
                                                                      liv
                                                                      plan
                                                                      Insecure
  ...
  httpsTid <- for tls $ \tlsOpts ->
    async $ io $ W.runTLSSocket tlsOpts httpsOpts httpsSock $ app env
                                                                  sId
                                                                  liv
                                                                  plan
                                                                  Secure
...
serv king plan isFake = (initialEvents, runHttpServer)
 where
  initialEvents :: [Ev]
  ...
  restart (Drv var) conf = do
    ...
    res <-
      fromEither =<< restartService var (startServ isFake conf plan) killServ
