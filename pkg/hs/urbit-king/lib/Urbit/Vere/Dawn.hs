...
provider =
  HttpProvider "https://mainnet.infura.io/v3/196a7f37c7d54211b4a07904ec73ad87"
...
passFromEth enc aut sut = Pass (decode aut) (decode enc)
  where decode = Ed.PublicKey . bytes32ToBS
...
withAzimuth :: Quantity -> Address -> DefaultAccount Web3 a -> Web3 a
withAzimuth bloq azimuth action =
  withAccount () $ withParam (to .~ azimuth) $ withParam
    (block .~ BlockWithNumber bloq)
    action

-- Retrieves the EthPoint information for an individual point.
retrievePoint :: Quantity -> Address -> Ship -> Web3 EthPoint
retrievePoint bloq azimuth ship = withAzimuth bloq azimuth $ do
  (encryptionKey, authenticationKey, hasSponsor, active, escapeRequested, sponsor, escapeTo, cryptoSuite, keyRevision, continuityNum) <-
    AZ.points (fromIntegral ship)

  let escapeState =
        if escapeRequested then Just $ Ship $ fromIntegral escapeTo else Nothing
...
retrieveGalaxyTable bloq azimuth =
 ...
 where
  getRow idx = do
    (encryptionKey, authenticationKey, _, _, _, _, _, cryptoSuite, keyRev, continuity) <-
      AZ.points idx
    pure
      ( fromIntegral idx
...
readAmesDomains bloq azimuth = withAzimuth bloq azimuth $ nub <$> mapM
  getTurf
  [0 .. 2]
 where
  getTurf idx = Turf . fmap Cord . reverse . splitOn "." <$> AZ.dnsDomains idx
...
validateShipAndGetImmediateSponsor block azimuth (Seed ship life ring oaf) =
 ...
 where
  validateComet = do
    ...
    when (ship /= shipFromPass) $ fail
      (  "comet name doesn't match fingerprint "
      ++ show ship
      ++ " vs "
      ++ show shipFromPass
      )
    when (life /= 1) $ fail ("comet can never be re-keyed")
    pure (shipSein ship)
  ...
  validateRest = do
    ...
    case epNet whoP of
      ...
      Just (netLife, pass, contNum, (hasSponsor, who), _) -> do
        when (netLife /= life) $ fail
          (  "keyfile life mismatch; keyfile claims life "
          ++ show life
          ++ ", but Azimuth claims life "
          ++ show netLife
          )
...
tryMineComet ships seed = if member shipSponsor ships
...
mineComet ships = loop
 where
  loop eny = case (tryMineComet ships eny) of
    Nothing -> loop (eny + 1)
    Just x  -> x
