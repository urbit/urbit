...
hearEv p a bs = EvBlip $ BlipEvAmes $ AmesEvHear () dest (MkBytes bs)
  where dest = EachNo $ Jammed $ AAIpv4 (Ipv4 a) (fromIntegral p)
...
ames inst who isFake enqueueEv stderr = (initialEvents, runAmes)
 where
  ...
  sendPacket AmesDrv {..} Fake      dest bs = do
    when (okayFakeAddr dest) $ atomically $ writeTQueue
      aSendingQueue
      ((localhostSockAddr Fake dest), bs)
  ...
  sendingThread queue (Just socket) = forever $ do
  ...
  galaxyResolver
    ...
    -> RIO e ()
  galaxyResolver galaxy turfVar incoming outgoing = loop Nothing Time.unixEpoch
   where
    ...
    checkIP :: Maybe SockAddr -> Time.Wen -> RIO e (Maybe SockAddr, Time.Wen)
      ...
      case listIPs of
        ...
        (y : ys) -> do
          ...
          when (sockaddr /= prevIP)
            ...
            ++ ": ip "
            ++ (tshow $ addrAddress y)
          logDebug $ displayShow ("(ames) Looked up ", hostname, portstr, y)
          pure sockaddr
