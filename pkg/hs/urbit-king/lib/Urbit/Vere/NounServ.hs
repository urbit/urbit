...
wsConn
  :: (FromNoun i, ToNoun o, Show i, Show o, HasLogFunc e)
  => Utf8Builder
  -> TBMChan i
  -> TBMChan o
  -> WS.Connection
...
wsClient
  :: forall i o e
   . (ToNoun o, FromNoun i, Show o, Show i, HasLogFunc e)
  => Text
  -> W.Port
  -> RIO e (Client i o)
wsClient pax por = do
  ...
  tid <- io $ async $ WS.runClient "127.0.0.1" por (unpack pax) $ \con ->
    WS.withPingThread con 15 (pure ())
...
wsServApp
  :: (HasLogFunc e, ToNoun o, FromNoun i, Show i, Show o)
  => (Conn i o -> STM ())
...
wsServer
  :: forall i o e
   . (ToNoun o, FromNoun i, Show i, Show o, HasLogFunc e)
...
wsServer = do
  ...
  tid <- async $ do
    ...
    io $ WS.runServer "127.0.0.1" 9999 $ runRIO env . wsServApp
      (writeTBMChan con)
