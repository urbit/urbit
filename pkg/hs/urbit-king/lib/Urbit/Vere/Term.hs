...
rioAllocaBytes :: (MonadIO m, MonadUnliftIO m) => Int -> (Ptr a -> m b) -> m b
rioAllocaBytes size action =
  withRunInIO $ \run -> allocaBytes size $ \x -> run (action x)
...
connClient c = Client { give = Serv.cSend c, take = Serv.cRecv c }
...
instance HasConfigDir HackConfigDir where
  configDirL = hcdPax
...
-}
localClient
  :: forall e . HasLogFunc e => STM () -> RAcquire e (T.TSize, Client)
localClient doneSignal = fst <$> mkRAcquire start stop
 where
  ...
  stop :: HasLogFunc e => ((T.TSize, Client), Private) -> RIO e ()
   ...
   where
    ...
    doSpin ls@LineState {..} mTxt = do
      ...
      delay   <- pure $ case mTxt of
        Nothing -> 0
        Just _  -> if (gap current lsPrevEndTime ^. microSecs) < _spin_idle_us
          then _spin_warm_us
          else _spin_cool_us
    ...
    unspin ls@LineState {..} = do
      ...
      ls      <- if not lsSpinFirstRender then termRefreshLine ls else pure ls
    ...
    spin ls@LineState {..} = do
      ...
      pure $ ls { lsSpinFirstRender = False, lsSpinFrame = newFrame }
  ...
  termShowCursor ls@LineState {..} {-line pos)-}
                                   newPos = do
    if newPos < lsCurPos
      then do
        T.cursorLeft (lsCurPos - newPos)
        pure ls { lsCurPos = newPos }
      else if newPos > lsCurPos
        then do
  ...
  readTerminal
    ...
    -> (RIO e ())
    -> RIO e ()
  readTerminal rq wq bell = rioAllocaBytes 1
    $ \buf -> loop (ReadData buf False False mempty 0)
   where
    loop :: ReadData -> RIO e ()
    loop rd@ReadData {..} = do
                        ...
                        -- ETX (^C)
                          ...
                          then do
                            ...
                            -- start the utf8 accumulation buffer
                              loop rd
                                { rdUTF8      = singleton w
                                , rdUTF8width = if w < 224
                                                  then 2
                                                  else if w < 240 then 3 else 4
                                }
...
term (tsize, Client {..}) shutdownSTM king enqueueEv = (initialEvents, runTerm)
 where
  T.TSize wi hi = tsize
