...
toNetworkConfig CLI.Opts {..} = NetworkConfig
  { ncNetworking = if (oDryRun || isJust oDryFrom)
                     then NetworkNone
                     else if oOffline
                       then NetworkNone
                       else if oLocalhost
                         then NetworkLocalhost
                         else NetworkNormal
...
runOrExitImmediately
  ...
  -> MVar ()
  -> RIO e ()
runOrExitImmediately getPier oExit mStart = rwith getPier
  $ if oExit then shutdownImmediately else runPier
 where
...
runRAcquire
  :: (MonadUnliftIO (m e), MonadIO (m e), MonadReader e (m e))
  => RAcquire e a
  -> m e a
...
collectAllFx top = do
  ...
  rwith collectedFX $ \() -> logTrace "Done collecting effects!"
 where
  tmpDir :: FilePath
  tmpDir = top </> ".tmpdir"
...
replayPartEvs top last = do
  logTrace $ display $ pack @Text top
  fetchSnapshot
  rwith replayedEvs $ \() -> logTrace "Done replaying events!"
 where
  fetchSnapshot :: RIO e ()
  fetchSnapshot = do
...
validateNounVal
  :: (HasLogFunc e, Eq a, ToNoun a, FromNoun a) => a -> RIO e ByteString
validateNounVal inpVal = do
  ...
  unless (inpVal == outVal) $ error "Value fails test: x == cue (jam x)"
...
newShip CLI.New {..} opts
 ...
 where
  ...
  runTryBootFromPill pill name ship bootEvent = do
    ...
    io $ runPierApp pierConfig networkConfig True $ tryBootFromPill True
                                                                    pill
                                                                    nLite
                                                                    flags
                                                                    ship
...
runShip (CLI.Run pierPath) opts daemon = do
 ...
 where
  runPier mStart = runPierApp pierConfig networkConfig daemon $ tryPlayShip
    (CLI.oExit opts)
...
connTerm pier = Term.runTerminalClient pier

--------------------------------------------------------------------------------

checkFx :: HasLogFunc e => FilePath -> Word64 -> Word64 -> RIO e ()
checkFx pierPath first last = rwith (Log.existing logPath)
  $ \log -> runConduit $ streamFX log first last .| tryParseFXStream
  where logPath = pierPath <> "/.urb/log"
