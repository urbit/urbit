...
module Urbit.King.EventBrowser
  ( run
  )
where
...
run log = do
  ...
  loop cycle las las
 where
  failRead cur = putStrLn ("ERROR: Failed to read event: " <> tshow cur)
...
deleteFrom log las cur = do
 ...
 where
  ...
  question = if las == cur
    then mconcat
      ["    This will permanently delete the last event (#", tshow las, ")\n"]
    else mconcat
      [ "    This will permanently delete all events in (#"
      , tshow cur
      , " - #"
      , tshow las
      , ")\n"
...
showEffectsTeaser (Just fx) = putStrLn $ mconcat
  ["    [", tshow (length fx), " collected effects. Press 'f' to view]\n"]
