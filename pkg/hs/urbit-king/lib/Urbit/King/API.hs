...
kingServer is = mkRAcquire (startKing is) (cancel . kServer)
 where
...
-}
kingAPI :: (HasConfigDir e, HasLogFunc e) => RAcquire e King
kingAPI = do
...
serveTerminal env api word = WS.websocketsOr WS.defaultConnectionOptions
                                             wsApp
                                             fallback
 where
  ...
  wsApp pen = atomically (readTVar api) >>= \case
    ...
    Just sp -> do
      ...
      let doit = runRIO env $ NounServ.wsConn "NOUNSERV (wsServ) " inp out wsc
...
app env api req respond = case W.pathInfo req of
  ["terminal", session] -> do
    session :: Word <- evaluate $ read $ unpack session
    serveTerminal env api session req respond
  ["status"] -> respond $ W.responseLBS H.status200 [] "{}"
  _          -> respond $ W.responseLBS H.status404 [] "No implemented"
