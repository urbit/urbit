...
addErrTag tag exp = InfixE (Just $ AppE (VarE 'named) str)
                           (VarE (mkName "."))
                           (Just exp)
  where str = LitE $ StringL tag
...
enumFromAtom cons = LamE [VarP x] body
 where
  ...
  mkMatch   = \(tag, nm) -> Match (SigP (LitP $ StringL tag) (ConT ''Text))
...
vodFromNoun = LamE [WildP] body
 where
  body =
    AppE (VarE 'fail) $ LitE $ StringL "Can't FromNoun on uninhabited data type"
...
unexpectedTag expected got = applyE
  (VarE 'mappend)
  [LitE (StringL prefix), AppE (VarE 'unpack) got]
 where
...
taggedFromNoun cons = LamE [VarP n] (DoE [getHead, getTag, examine])
 where
  ...
  getHead      = BindS (TupP [VarP h, VarP t]) $ AppE (VarE 'parseNoun) (VarE n)
  ...
  matches = mkMatch <$> cons
  mkMatch = \(tag, (n, tys)) ->
    let body = AppE (addErrTag ('%' : tag) (tupFromNoun (n, tys))) (VarE t)
...
tagNoun :: String -> Exp
tagNoun = AppE (VarE 'textToUtf8Atom) . LitE . StringL
...
tupToNoun cons = LamCaseE [mkMatch cons]
 where
  ...
  mkMatch (nm, tys) = Match (ConP nm params) (NormalB body) []
   where
    vars   = (zip tys ['a' ..]) <&> (mkName . singleton . snd)
    params = VarP <$> vars
    body   = tup vars

sumToNoun :: [(String, Name)] -> [(String, ConInfo)] -> Exp
sumToNoun a c = LamCaseE (mixed <&> uncurry mkMatch)
 where
  mixed = mconcat [a <&> \(x, y) -> (x, Left y), c <&> \(x, y) -> (x, Right y)]
