...
import Prelude        ((!!))
import RIO.FilePath   (joinPath, splitDirectories, takeBaseName, takeDirectory,
                       takeExtension, (<.>))
...
uTypeAddDots = reverse . go . reverse
 where
  go s = if null tel then hed else hed <> "." <> go tel
   where
    hed = take 5 s
    tel = drop 5 s
...
instance FromNoun a => FromNoun (Lenient a) where
  parseNoun n = (GoodParse <$> parseNoun n) <|> fallback
   where
    fallback = fromNounErr n & \case
...
instance ToNoun Octs where
  toNoun (Octs bs) = toNoun (int2Word (length bs), bytesAtom bs)
   where
    int2Word :: Int -> Word
...
instance ToNoun Word8   where
  toNoun = wordToNoun
instance ToNoun Word16  where
  toNoun = wordToNoun
instance ToNoun Word32  where
  toNoun = wordToNoun
instance ToNoun Word64  where
  toNoun = wordToNoun
instance ToNoun Word128 where
  toNoun = wordToNoun
instance ToNoun Word256 where
  toNoun = wordToNoun
instance ToNoun Word512 where
  toNoun = wordToNoun

instance FromNoun Word    where
  parseNoun = named "Word" . nounToWord
instance FromNoun Word8   where
  parseNoun = named "Word8" . nounToWord
instance FromNoun Word16  where
  parseNoun = named "Word16" . nounToWord
instance FromNoun Word32  where
  parseNoun = named "Word32" . nounToWord
instance FromNoun Word64  where
  parseNoun = named "Word64" . nounToWord
instance FromNoun Word128 where
  parseNoun = named "Word128" . nounToWord
instance FromNoun Word256 where
  parseNoun = named "Word256" . nounToWord
instance FromNoun Word512 where
  parseNoun = named "Word512" . nounToWord
...
instance FromNoun a => FromNoun (Maybe a) where
  parseNoun = named "Maybe" . \case
    ...
    where unexpected s = fail ("Expected unit value, but got " <> s)
...
instance (FromNoun a, FromNoun b) => FromNoun (a, b) where
  parseNoun n = named ("(,)") $ do
    case n of
      A _   -> shortRec 0
      C x y -> do
        (,) <$> named "1" (parseNoun x) <*> named "2" (parseNoun y)
...
instance (FromNoun a, FromNoun b, FromNoun c) => FromNoun (a, b, c) where
  parseNoun n = named "(,,)" $ do
    case n of
      ...
      C x (A _) -> shortRec 1
      C x (C y z) ->
        (,,) <$> named "1" (parseNoun x) <*> named "2" (parseNoun y) <*> named
          "3"
          (parseNoun z)
