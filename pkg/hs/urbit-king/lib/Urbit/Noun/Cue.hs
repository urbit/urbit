...
doGet m bs =
  unsafePerformIO $ try $ BS.unsafeUseAsCStringLen bs $ \(ptr, len) -> do
    ...
    let sz     = max 50 $ min 10_000_000 $ length bs `div` 6
    tbl           <- H.newSized sz
...
{-# INLINE getPos #-}
getPos :: Get Word
getPos = Get $ \_ _ s -> pure (GetResult s (pos s))
...
advance !n = Get $ \_ _ s -> do
  ...
  let
    newUsed = n + usedBits s
    newS    = s
      { pos      = pos s + n
      , usedBits = newUsed `mod` 64
      , currPtr  = plusPtr (currPtr s) (8 * (fromIntegral (newUsed `div` 64)))
      }
...
guardInfinite end cur s = when (cur >= (end `plusPtr` 16)) $ do
...
peekUsedBits = debugMId "peekUsedBits" $ do
...
dAtomBits !(fromIntegral -> bits) = do
  debugMId ("dAtomBits(" <> show bits <> ")") $ do
    fmap wordsAtom $ VP.generateM bufSize $ \i -> do
      debugM (show i)
...
peekWord = do
  debugMId "peekWord" $ do
    ...
    debugM
      (  "\t"
      <> (take 10 $ reverse $ printf "%b" (fromIntegral res :: Integer))
      <> ".."
      )
    pure res
