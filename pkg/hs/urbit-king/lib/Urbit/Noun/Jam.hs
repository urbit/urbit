...
jamBS n = doPut bt sz (writeNoun n)
  where (sz, bt) = unsafePerformIO (compress n)
...
writeWord wor = do
  ...
  flush
  update $ \s -> s { pos = 64 + pos, reg = shiftR wor (64 - off) }
...
doPut !tbl !sz m = unsafePerformIO $ do
        -- traceM "doPut"
