...
named nm (Parser cb) = Parser $ \path kf ks -> cb (nm : path) kf ks

instance Monad Parser where
  m >>= g = Parser $ \path kf ks ->
    let ks' a = runParser (g a) path kf ks in runParser m path kf ks'
  return = pure
  fail   = Fail.fail
...
instance Functor Parser where
  fmap f m =
    Parser $ \path kf ks -> let ks' a = ks (f a) in runParser m path kf ks'
...
instance MonadPlus Parser where
  ...
  mplus a b = Parser $ \path kf ks ->
    let kf' _ _ = runParser b path kf ks in runParser a path kf' ks
...
instance Show BadNoun where
  show (BadNoun pax msg) =
    mconcat ["(BadNoun ", show (intercalate "." pax), " ", show msg, ")"]
...
parseNounUtf8Atom n = named "utf8-atom" $ do
