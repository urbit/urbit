...
renderProgressBarBenchmark width done = bench name $ nf
  (\(s, p, t) -> renderProgressBar s p t)
...
labelBenchmark labelName label progress = bench name
  $ nf (\(p, t) -> runLabel label p t) (progress, someTiming)
 where
  name =
    "label/"
      <> labelName
