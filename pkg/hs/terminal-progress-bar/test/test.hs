...
import           "test-framework" Test.Framework                       (Test, defaultMainWithOpts,
                                                                        interpretArgsOrExit,
                                                                        testGroup)
...
main :: IO ()
main = do
  opts <- interpretArgsOrExit =<< getArgs
...
tests =
  [ testGroup
    ...
    , eqTest "pre & post" "pre [] post" (msg "pre") (msg "post") 0
      $ Progress 0 0 ()
    ]
  , testGroup
    "Bar fill"
...
labelTest testName label progress expected =
  testCase testName $ assertEqual expectationError expected $ runLabel
    label
    progress
    someTiming
...
renderDurationTest dt expected =
  testCase ("renderDuration " <> show dt)
    $ assertEqual expectationError expected
    $ renderDuration dt

eqTest
  :: String -> TL.Text -> Label () -> Label () -> Int -> Progress () -> Test
eqTest name expected mkPreLabel mkPostLabel width progress =
 ...
 where
  ...
  style :: Style ()
  style = defStyle { stylePrefix  = mkPreLabel
