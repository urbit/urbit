...
toCopy = fst . runWriter . go (\v -> error "toCopy: free variable")
 where
  ...
  lam s env ctor manner = writer
    (ctor (rude . env <$> Set.toAscList usedLexicals) ce, usedLexicals)
   where
...
copyToNock = go $ \v -> error "copyToNock: free variable"
 where
  ...
  go env = \case
    ...
    CFix vs e ->
      N7 (lam (map (go env . CVar) vs) (go (lamEnv vs) e)) (N2 (N0 1) (N0 1))
  ...
  jet a ef = NC (N1 (A 11)) (NC (N1 (C (A FastAtom) (C (A 1) (A a)))) ef)
...
decrement = lam "a" $ App
  (fix "f" $ lam "b" $ Ift (Eql (Var "a") (Suc (Var "b")))
                           (Var "b")
                           (App (Var "f") (Suc (Var "b")))
  )
  (Atm 0)
