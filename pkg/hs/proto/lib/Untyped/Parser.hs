...
gap :: Parser ()
gap =
  choice [char ' ' >> void (some spaceChar), newline >> void (many spaceChar)]
...
sym :: Parser Sym
sym = bucSym <|> pack <$> some alpha where bucSym = char '$' *> pure ""
...
tape :: Parser Text
tape = do
  between (char '"') (char '"') $ pack <$> many
    (label "tape char" (anySingleBut '"'))

cord :: Parser Text
cord = do
  between (char '\'') (char '\'') $ pack <$> many
    (label "cord char" (anySingleBut '\''))
...
irregular = inWideMode $ choice
  [Tupl <$> tuple cst, IncrIrr <$> inc, uncurry IsEqIrr <$> equals, appIrr]

-- Runes -----------------------------------------------------------------------

pat :: Parser Pat
pat = choice [PatTag <$> tag, char '*' $> PatTar]
...
parse txt = runParser (evalStateT hoonFile Tall) "stdin" txt & \case
