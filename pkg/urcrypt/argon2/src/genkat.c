...
void
initial_kat(const uint8_t*        blockhash,
            ...
            argon2_type           type)
{
  ...
  if ( blockhash != NULL && context != NULL ) {
    ...
    printf(
      "Memory: %u KiB, Iterations: %u, Parallelism: %u lanes, Tag "
      "length: %u bytes\n",
      context->m_cost,
      context->t_cost,
      context->lanes,
...
void
print_tag(const void* out, uint32_t outlen)
{
  unsigned i;
  if ( out != NULL ) {
    printf("Tag: ");
...
void
internal_kat(const argon2_instance_t* instance, uint32_t pass)
{
  if ( instance != NULL ) {
    uint32_t i, j;
    printf("\n After pass %u:\n", pass);
...
static void
generate_testvectors(argon2_type type, const uint32_t version)
{
