...
int
allocate_memory(const argon2_context* context,
                uint8_t**             memory,
                size_t                num,
                size_t                size)
{
...
void
free_memory(const argon2_context* context,
            uint8_t*              memory,
            size_t                num,
            size_t                size)
{
...
static int
fill_memory_blocks_mt(argon2_instance_t* instance)
{
  uint32_t                r, s;
...
initial_hash(uint8_t* blockhash, argon2_context* context, argon2_type type)
{
  ...
  if ( context->secret != NULL ) {
    blake2b_update(&BlakeHash,
                   (const uint8_t*)context->secret,
                   context->secretlen);
...
initialize(argon2_instance_t* instance, argon2_context* context)
{
  ...
  /* 1. Memory allocation */
  result = allocate_memory(context,
                           (uint8_t**)&(instance->memory),
                           instance->memory_blocks,
                           sizeof(block));
  if ( result != ARGON2_OK ) {
