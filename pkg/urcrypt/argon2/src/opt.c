           ...
           int          with_xor)
{
  ...
  if ( with_xor ) {
    for ( i = 0; i < ARGON2_OWORDS_IN_BLOCK; i++ ) {
      state[i]
        = _mm_xor_si128(state[i],
                        _mm_loadu_si128((const __m128i*)ref_block->v + i));
      block_XY[i]
        = _mm_xor_si128(state[i],
                        _mm_loadu_si128((const __m128i*)next_block->v + i));
    }
  }
  else {
    for ( i = 0; i < ARGON2_OWORDS_IN_BLOCK; i++ ) {
...
fill_segment(const argon2_instance_t* instance, argon2_position_t position)
{
        ...
        ++i, ++curr_offset, ++prev_offset )
  {
     ...
     */
    position.index = i;
    ref_index      = index_alpha(instance,
                            &position,
