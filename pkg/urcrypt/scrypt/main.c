...
#define REF2                                                                   \
  "7023bdcb3afd7348461c06cd81fd38ebfda8fbba904f8e3ea9b543f6545da1f2d543295561" \
  "3f0fcf62d49705242a9af9e61e85dc0d651e40dfcf017b45575887"
...
*/
  ...
  printf(
    "TEST ONE: Direct call to reference function with password 'password' and "
    "salt 'NaCL'\n");
  ...
  retval
    = libscrypt_hexconvert(hashbuf, sizeof(hashbuf), outbuf, sizeof(outbuf));
  if ( !retval ) {
  ...
  printf(
    "TEST FOUR: Direct call to reference function with pleaseletmein password "
    "and SodiumChloride as salt\n");

  /* Tests 4-6 repeat tests 1-3 with a different reference vector */

  retval = libscrypt_scrypt((uint8_t*)"pleaseletmein",
                            strlen("pleaseletmein"),
                            (uint8_t*)"SodiumChloride",
  ...
  */
  ...
  retval
    = libscrypt_b64_encode(hashbuf, sizeof(hashbuf), outbuf, sizeof(outbuf));
  if ( retval == -1 ) {
    printf("TEST SEVEN FAILED\n");
    exit(EXIT_FAILURE);
  }
  retval = libscrypt_b64_encode((unsigned char*)"SodiumChloride",
                                strlen("SodiumChloride"),
                                saltbuf,
                                sizeof(saltbuf));
  if ( retval == -1 ) {
   ...
   * int crypto_scrypt_mcf(uint32_t N, uint32_t r, uint32_t p, char *salt, char
   * *hash, char *mcf); Returns 0 on error, most likely reason is log2(N) not an
   * integer.
   */
  ...
  if ( retval == 0 ) {
    printf(
      "TEST NINE: FAILED, claimed pleaseletmein hash claimed did not verify\n");
    exit(EXIT_FAILURE);
  }
  ...
  retval = libscrypt_b64_encode((uint8_t*)saltbuf,
                                SCRYPT_SALT_LEN,
                                outbuf,
                                sizeof(outbuf));
  if ( retval == -1 ) {
  ...
  printf(
    "TEST TWELVE: SUCCESSFUL. Received the following from simple hash:\n%s\n",
    outbuf);
