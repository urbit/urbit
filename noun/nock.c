  ...
  while ( i_w-- > 0 ) {
    ...
    if ( c3y == u3ud(op) ) {
      ...
      switch ( cod ) {
        ...
        case SBIP: case SBIN:
          sod_y = (c3_y) cod;
          wil_l = (c3_l) ip_l + ((c3_l) u3t(op));
          was_l = (c3_l) i_w;
          c3_assert(c3y == u3a_is_cat(wil_l));
          c3_assert(c3y == u3a_is_cat(was_l));
          sil   = u3nc(u3nt(sod_y, wil_l, was_l), sil);
          break;
