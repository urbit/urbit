...
Tile generator. Produces a tuple of two nouns of the types passed in.
...
Used by compiler, and wherever typed date is worked with explicitly
...
Used where lists are convenient
...
Line list, instead of control characrters
...
Adress in subject
    ...
    ~zod/try=> `wain`/som/del/rok
    <|som del rok|>
    ~zod/try=> `wain`(lore (,@ .^(cx//=main=/bin/tree/hoon)))
    <|
      !:
      ::  /===/bin/tree/hoon
      |=  ^  
...
Forcibly pull result out of edge
...
Tile constructor: treap of pairs
...
Tile consturctor: treap of type, ordered
...
Tile consturctor: treap of type, unique
