...
*/
  ...
  _lexs(u3_loss* loc_u)
  {
    if ( 0 == loc_u->kct_w ) {
      return u3_nul;
    } else return u3lb_flop(_lext(loc_u, loc_u->kad[loc_u->kct_w - 1]));
  }
  ...
  {
    loc_u->hel = hel;
    loc_u->lel_w = u3lb_lent(u3k(hel));

    //  Read hev into array.
    {
      c3_w i_w;

      loc_u->hev = c3_malloc(u3lb_lent(u3k(hev)) * sizeof(u3_noun));
    ...
    loc_u->sev = u3_nul;
    {
      ...
      for ( i_w = 0; i_w < loc_u->lev_w; i_w++ ) {
        ...
        hav = u3ldb_get(u3k(loc_u->sev), u3k(how));
        teg = u3nc(u3i_words(1, &i_w),
                   (hav == u3_none) ? u3_nul : hav);
        loc_u->sev = u3ldb_put(loc_u->sev, u3k(how), teg);
      }
    }
  }
  ...
  _loss(u3_loss* loc_u)
  {
    while ( u3_nul != loc_u->hel ) {
      ...
      u3_noun guy   = u3ldb_get(u3k(loc_u->sev), u3k(i_hel));

      if ( u3_none != guy ) {
        u3_noun gay = u3lb_flop(guy);
