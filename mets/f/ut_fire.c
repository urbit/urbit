...
*/
  ...
  _fire_mull(u3_noun van,
             ...
             u3_noun gen)
  {
    u3_noun rib = u3r_at(u3gfu_van_rib, van);
    ...
    if ( c3y == u3gdi_has(rib, key) ) {
      ret = c3y;
    }
    else {
      u3_noun rob = u3gdi_put(rib, key);
      u3_noun von = u3i_molt(u3k(van),
                             u3gfu_van_rib,
                             u3k(rob),
                             0);
      u3_noun mul = u3gfu_mull(von, sut, c3__noun, dox, gen);
             ...
             u3_noun gat)
  {
           ...
           (c3n == u3r_cell(gat, &h_gat, &t_gat)) )
    {
      ...
      if ( c3__ash == u3h(gat) ) {
        if ( (c3y == vet) &&
             (c3n == u3gfu_nest(van, qq_typ, c3y, p_typ)) )
        {
#if 0
          u3_noun dun = u3gfu_dunq(van, "need", qq_typ);
          u3_noun niz = u3gfu_dunq(van, "have", p_typ);
...
#if 0
        u3_noun dun = u3gfu_dunq(van, "wild", typ);
        u3_noun niz = u3gfu_dunq(van, "tame", dox);
