...
*/
             ...
             u3_noun got)
  {
    if ( c3y == u3ud(got) ) {
      ...
      case 2: {
        u3_noun vet = u3r_at(u3gfu_van_vet, van);
        u3_noun hed, tal, deh, lat, ret;

        if ( c3y == vet ) {
          u3_noun cel = u3nt(c3__cell, c3__noun, c3__noun);

          if ( c3n == u3gfu_nest(van, cel, c3y, sut) ) {
            return u3m_bail(c3__fail);
          }
          u3z(cel);
        }
        hed = u3gfu_peek(van, sut, c3__both, 2);
        tal = u3gfu_peek(van, sut, c3__both, 3);
        deh = _cqfu_conk(van, hed, u3h(u3t(got)));
        lat = _cqfu_conk(van, tal, u3t(u3t(got)));

        ret = u3gf_cell(deh, lat);
